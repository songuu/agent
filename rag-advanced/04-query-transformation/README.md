...
- [ ] 用 **routing / decomposition / step-back** 先判断查询类型、拆开复合意图、抽象成更通用的问题，再决定走哪条检索策略。
...
### 1) 先做查询规划：routing / decomposition / step-back

查询改写不只有「让 LLM 多写几句」。真正的 RAG 查询入口通常先做三件确定性规划：

- **routing**：这题属于价格、配额、身份、SLA、保留策略，还是通用检索？
- **decomposition**：一句话里如果有多个意图，先拆成多个子问题，分别检索。
- **step-back**：把过窄、口语化的问题抽象成更通用的检索问题，例如「我的数据会被删掉吗」→「数据生命周期、归档、保留期和删除规则是什么」。

代码里对应 `planQuery()`，它是纯函数，不需要 key，可进 smoke：

```ts
const planned = planQuery("专业版多少钱，同时导出额度是多少？");
console.log(planned.route.route.label);
console.log(planned.subQueries);
console.log(planned.stepBackQuery);
```

### 2) multiQuery：用「多措辞并集」对抗漏召回
...
graph LR
  ...
  n_cragagentic_rewrite_loop["改写重试循环（第rag-agentic章）"]
  ...
  class n_c08_embedding,n_c09_topk_retrieval,n_cragagentic_rewrite_loop cross;
...
- `改写重试循环` —**应用**→ `多查询改写`（第 rag-agentic 章）
