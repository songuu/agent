...
assert.match(source, /scheduleZoomableDiagramsInstall/);
assert.match(source, /installAfterHydration/);
assert.match(source, /DOMContentLoaded/);
assert.match(source, /requestAnimationFrame\(\(\) => installZoomableDiagrams\(\)\)/);
assert.doesNotMatch(source, /if \(typeof window !== "undefined"\) {\s*installZoomableDiagrams\(\);/);
assert.match(source, /diagram-zoom-viewport/);
assert.match(source, /diagram-zoom-toolbar/);
assert.match(source, /可缩放流程图或思维导图/);
assert.match(source, /content\.replaceChildren\(sourceSvg\.cloneNode\(true\)\)/);
assert.match(source, /viewport\.remove\(\)/);
assert.match(source, /diagram\.hidden = true/);
assert.match(source, /sourceObserver\.observe\(diagram, \{ childList: true, subtree: true \}\)/);
assert.match(source, /openDiagramOverlay\(content\)/);
assert.doesNotMatch(source, /surface\.append\(diagram\)/);
...
assert.match(source, /normalizeDiagramSvgSize\(content\)/);
assert.match(source, /SVG_BOUNDS_PADDING = 8/);
assert.match(source, /expandSvgViewBoxToVisibleBounds/);
assert.match(source, /svg\.setAttribute\("viewBox", viewBoxValue\)/);
assert.match(source, /viewBox\.width/);
assert.match(source, /svg\.style\.width/);
assert.match(source, /svg\.style\.overflow = "hidden"/);
assert.match(source, /dataset\.diagramZoomNormalized = "true"/);
assert.match(source, /getDiagramBounds/);
assert.match(source, /diagramZoomNormalized === "true"/);
assert.match(source, /return \{ x: 0, y: 0, width: svgRect\.width, height: svgRect\.height \}/);
...
assert.match(styles, /\.diagram-zoom-viewport\s*{[^}]*max-width:\s*100%/s);
assert.match(styles, /\.diagram-zoom-viewport\s*{[^}]*overflow:\s*hidden/);
...
assert.match(styles, /\.diagram-zoom-surface\s*{[^}]*max-width:\s*100%/s);
assert.match(styles, /\.diagram-zoom-content\s+svg/);
assert.match(styles, /\.diagram-zoom-content\s+\.nodeLabel/);
assert.match(styles, /\.diagram-zoom-content\s+\.node\s+rect/);
assert.match(styles, /max-width:\s*none/);
assert.match(styles, /\.diagram-zoom-content\s+svg\s*{[^}]*overflow:\s*hidden/);
assert.match(styles, /\.diagram-zoom-button/);
// 工具栏默认隐藏，只在具体图表内容 hover、工具栏 hover、键盘 focus 时显示。
assert.match(styles, /\.diagram-zoom-toolbar\s*{[^}]*opacity:\s*0/s);
assert.match(styles, /\.diagram-zoom-toolbar\s*{[^}]*pointer-events:\s*none/s);
assert.match(styles, /\.diagram-zoom-viewport:has\(\.diagram-zoom-content:hover\)\s*>\s*\.diagram-zoom-toolbar/s);
assert.match(styles, /\.diagram-zoom-viewport:focus-within\s*>\s*\.diagram-zoom-toolbar/s);
assert.match(styles, /\.diagram-zoom-overlay-panel:has\(\.diagram-zoom-content:hover\)\s*>\s*\.diagram-zoom-toolbar/s);
assert.match(styles, /opacity:\s*1;\s*\n\s*pointer-events:\s*auto/s);
// 溢出渐隐提示（不静默裁切）。
