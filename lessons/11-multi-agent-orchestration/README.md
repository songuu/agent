...
graph TB
  ...
  n_crev_crew["评审团 supervisor (并行 fork-join)（第cap-review章）"]
  ...
  class n_c06_run_agent_loop,n_crev_crew,n_lgma_supervisor,n_lgma_worker_routing,n_lgma_order_independent_join cross;
...
- `评审团 supervisor (并行 fork-join)` —**深化**→ `编排拓扑选择`（第 cap-review 章）
- `评审团 supervisor (并行 fork-join)` —**应用**→ `审批与可观测`（第 cap-review 章）
