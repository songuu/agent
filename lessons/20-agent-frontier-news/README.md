...
> 所属阶段：**第七部分 · 前沿与生态**
> 预计用时：45 分钟 | 难度：⭐⭐☆☆☆
> 资料时间：截至 2026-06-17，基于课程知识图谱收录的官方文档、论文、GitHub release notes 与工程文章整理；阅读前建议复核原文发布日期。
...
- [ ] 按日期浏览 agent 前沿资料，知道每批资料为什么被收录。
- [ ] 用第 19 章的八层生态框架筛选文章，而不是被框架名词牵着走。
- [ ] 从文章详情中快速抓住来源、摘要、体系层、标签和原文入口。
...
> 本章不是新增一套理论，而是把持续变化的外部资料整理成可追溯阅读流。读图顺序：先看资料来源，再按体系层和日期筛，最后进入详情与原文。

```mermaid
flowchart LR
  A["外部资料\n官方文档 / 论文 / 工程博客"] --> B["知识图谱收录\n标题 / 来源 / 摘要 / 体系层"]
...
这页的文章库直接读取 Supabase `frontier_ecosystem_articles`，但唯一事实源仍是仓库里的 `knowledge-graph/data/graph.ts`。也就是说：

- 文章标题、来源、摘要和体系层先进入知识图谱；
- 生成脚本把它们派生为 Supabase seed；
...
日期不是装饰。agent 生态变化很快，资料的收集时间能帮助你判断：
...
> 下方文章库由页面直接从 Supabase `frontier_ecosystem_articles` 读取。标题、来源和“查看原文”均会打开原始资料；课程页只保留可追溯摘要，不复制原文全文。
