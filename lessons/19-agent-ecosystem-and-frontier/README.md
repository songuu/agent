...
- 来源：OpenAI · [OpenAI: The next evolution of the Agents SDK](https://openai.com/index/the-next-evolution-of-the-agents-sdk/) — OpenAI 官方产品文章：Agents SDK 向 sandbox execution、long-horizon tasks、durable harness 演进，是前沿趋势与可恢复 run contract 的来源 `blog`
...
- 来源：OpenAI · [OpenAI Docs · Evaluate agent workflows](https://developers.openai.com/api/docs/guides/agent-evals) — OpenAI 官方 agent workflow eval 指南，对应离线评估、trace/replay 与发布门治理 `doc`
...
- 来源：Anthropic · [Effective context engineering for AI agents](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents) — Anthropic 官方：上下文是有限资源，需主动裁剪、压缩和按需装配，对应 compiler、runtime 与长期任务压缩 `blog`
...
- 来源：GitHub Changelog · [GitHub Copilot weekly releases: August 3](https://github.blog/changelog/2026-08-07-github-copilot-weekly-releases-august-3) — GitHub Copilot 8 月 3 日周更把 cloud-agent reasoning level、CLI session/worktree/workflow polish、VS Code rewind improvements 和 tool-call duration observability 放在同一批更新里。信号是 coding agent 产品正在从单点模型能力转向可恢复会话、工作区隔离、执行耗时可见和模型推理预算可调的组合控制面。 `blog`
- 来源：GitHub Changelog · [Customize the reasoning level for Copilot cloud agent](https://github.blog/changelog/2026-08-06-customize-the-reasoning-level-for-copilot-cloud-agent) — GitHub 允许用户在 Copilot cloud agent 中为任务选择推理等级，把模型推理预算从隐式默认变成用户可见的任务级旋钮。信号是 coding agent 的质量、延迟和成本不应只由全局默认模型决定，而要按 issue/PR 风险、验证深度和预算上限配置。 `blog`
- 来源：GitHub Changelog · [Copilot code review effort levels are generally available](https://github.blog/changelog/2026-08-07-copilot-code-review-effort-levels-are-generally-available) — GitHub Copilot code review 的 effort levels GA，团队可在 faster/normal/deeper 等审查深度之间选择，权衡反馈速度、发现深层问题的概率与计算成本。信号是 AI reviewer 不应只有开关状态，生产集成需要按变更风险、测试覆盖、发布紧急度和人工 reviewer 负载配置审查强度。 `blog`
- 来源：GitHub Docs · [GitHub Copilot usage metrics docs: agent and IDE telemetry attribution](https://docs.github.com/en/copilot/concepts/copilot-usage-metrics/copilot-metrics) — GitHub Copilot usage metrics 文档把 Copilot 使用度量拆成 API、dashboard、code generation dashboard、impact dashboard 和 NDJSON export，并说明指标来自 IDE/CLI telemetry 与 server-side telemetry。信号是 agent 采用度、留存、成本和产出代理指标必须按具体 surface 与数据新鲜度归因，不能只看 IDE 补全、活跃用户或合并 PR。 `doc`
- 来源：GitHub Docs · [GitHub Copilot CLI enterprise MCP allowlist](https://docs.github.com/en/copilot/reference/copilot-cli-reference/cli-command-reference#enterprise-mcp-allowlist) — GitHub Copilot CLI 文档说明 enterprise MCP allowlist 会对非默认 MCP server 计算 fingerprint 并请求企业 allowlist evaluate endpoint；未获批准或 endpoint 不可达时 fail-closed 阻断连接。信号是 MCP 扩展能力从开发者便利进入企业治理面后，必须把 server allowlist、tool surface、认证、审计和默认拒绝策略作为基础设施配置，而不是交给每个 agent 会话临时判断。 `doc`
- 来源：Microsoft Agents for Python changelog · [Microsoft 365 Agents SDK for Python 1.3.0 release notes](https://github.com/microsoft/Agents-for-python/blob/main/changelog.md) — Microsoft 365 Agents SDK for Python 1.3.0 在开源 changelog 中记录了 header propagation middleware、LLM service registration、MSTeams hosting install extra、TeamsActivityHandler typing 和 TurnContext.services 等 runtime/hosting 改动。信号是企业 agent SDK 的成熟度不只看模型调用，还要看 host integration、身份/头传播、服务注册、类型契约和 Teams 渠道适配是否可回归。 `doc`
...
- 来源：npm / Cloudflare Docs · [Cloudflare Agents SDK 0.20.0 package and docs](https://www.npmjs.com/package/agents) — Cloudflare agents package 页面显示 0.20.0 latest，并把 state、SQL storage、schedule、human-in-the-loop、MCP、workflows 与 Durable Objects 连接到边缘 agent runtime。信号是生产 agent runtime 不只是调用模型，还要把长生命周期状态、定时触发、用户确认、工具协议和边缘执行身份作为一套部署契约。 `doc`
- 来源：Make Help Center · [Make AI Agent: Sub-agents and tool output filtering](https://help.make.com/ai-agents/make-ai-agent-sub-agents-and-tool-output-filtering) — Make AI Agent 增加 sub-agents 和 tool output filtering：主 agent 可以把子任务委托给专门 agent，并只把工具输出中必要字段继续传入上下文。信号是低代码 agent 产品也在把职责分解、上下文瘦身、权限边界和输出投影变成用户可配置的生产控制面。 `doc`
- 来源：Business Insider · [Disney steers developers from GitHub Copilot toward Codex, Claude Code and Cursor](https://www.businessinsider.com/disney-microsoft-github-copilot-openai-codex-ai-tools-claude-cursor-2026-7) — Business Insider 报道 Disney 内部把开发者从 GitHub Copilot 引导到 OpenAI Codex、Anthropic Claude Code 和 Cursor 等工具。信号不是某个工具必然胜出，而是企业 coding agent 采购正在从单一 Copilot 许可转为多工具组合治理，需要比较数据边界、仓库权限、审计、成本、员工迁移和模型/IDE 生态适配。 `blog`
- 来源：npm / OpenAI Agents JS Docs · [OpenAI Agents JS OpenAI provider 0.14.0 and AI SDK extension](https://www.npmjs.com/package/@openai/agents-openai) — OpenAI Agents JS 的 OpenAI provider package 进入 0.14.0，同时官方扩展文档提供 `toResponseModel` 等与 AI SDK 互操作的适配面。信号是 agent SDK 的版本治理不只看 core loop，还要把 provider 包、模型适配、工具调用、流式事件和外部 SDK 兼容一起纳入供应链与回归测试。 `doc`
- 来源：Claude Blog · [The new rules of context engineering for Claude 5 generation models](https://claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models) — Anthropic 把 context engineering 拆成 system prompt、conversation history、retrieved context、tool definitions、tool outputs 和可读 agent scratchpad，并强调长上下文、工具调用、token budget、clarity 与 retrieval noise 的取舍。信号是生产 agent 的效果越来越依赖上下文装配工程，而不是把更多文本塞进窗口。 `blog`
- 来源：Claude Blog / Datadog · [How Datadog built a universal machine tool for Claude Code](https://claude.com/blog/how-datadog-built-a-universal-machine-tool-for-claude-code) — Datadog 团队用 Temper 给 Claude Code 提供可隔离的云端开发环境，核心是把真实仓库、依赖、工具链和执行副作用放进受控 machine tool，而不是让 coding agent 直接污染本地环境。信号是 enterprise coding agent 的生产落点正在从 IDE 补全转向可审计、可复现、可回收的远程执行沙箱。 `blog`
- 来源：Salesforce Newsroom / Release Notes · [Salesforce Summer '26 Agentforce release: multi-agent orchestration and Tableau MCP](https://www.salesforce.com/news/stories/summer-2026-product-release-announcement/) — Salesforce Summer '26 release 把 Agentforce 的 multi-agent orchestration、Tableau MCP connector、agent testing center、observability 和 enterprise platform controls 放进同一批产品更新。信号是企业 agent 平台的竞争点正在转向跨 agent 编排、工具/数据协议接入、测试、监控和管理员默认控制，而不是单一聊天入口。 `blog`
- 来源：Cloudflare Changelog · [Cloudflare Agent traces for Think, Flue, and AI SDK](https://developers.cloudflare.com/changelog/post/2026-08-04-agent-tracing/) — Cloudflare Agents SDK 增加 agent traces：可在同一 trace 中查看 agent turn、model calls、tool runs、approvals、token usage 和 Workers runtime operations，并为 AI SDK v6/v7 提供 `wrapAISDK()`。信号是生产 agent 观测正在从普通日志推进到 turn/tool/model/runtime 同轨迹审计，同时需要明确消息和工具 payload 是否允许存储。 `doc`
- 来源：Cloudflare Changelog · [Preview: @cloudflare/computer agent runtime](https://developers.cloudflare.com/changelog/post/2026-08-03-cloudflare-computer/) — Cloudflare 预览开源 `@cloudflare/computer` agent runtime，为每个 agent 提供 SQLite-backed virtual filesystem，并在 isolate 与 full Linux container 间选择执行后端；read/write/edit/ls/exec 等操作被 gated、audited、observed。信号是 coding/automation agent 的 runtime 正从单一容器转向按任务选择执行平面，同时把文件系统、命令执行和审计作为默认边界。 `doc`
- 来源：Microsoft Agent Framework Blog · [Discover Agent Skills from MCP servers in .NET](https://devblogs.microsoft.com/agent-framework/discover-agent-skills-from-mcp-servers-in-net/) — Microsoft Agent Framework 的 .NET MCP skills 支持让 agent 通过 `Microsoft.Agents.AI.Mcp` 和 `UseMcpSkills(client)` 从 MCP server 发现并加载 Agent Skills，支持 `skill-md` 与 archive 两种分发形式。信号是 skill 正在从本地文件夹/应用内打包变成可集中发布、版本治理、按需加载的企业能力面，同时 archive 解包和脚本执行必须有受控目录与审批边界。 `blog`
- 来源：GitHub Changelog · [Copilot on web expands conversation controls](https://github.blog/changelog/2026-08-10-copilot-on-web-expands-conversation-controls) — GitHub Copilot on web 扩展 conversation controls，说明 coding agent 的 web 入口正在从单次聊天走向可管理的会话生命周期。信号是对话分支、上下文延续、历史整理和人工接管都需要被纳入产品交互与审计，而不是只验证最终代码建议。 `blog`
- 来源：GitHub Engineering Blog · [Using the GitHub Copilot SDK for Java](https://github.blog/engineering/using-the-github-copilot-sdk-for-java/) — GitHub Engineering 发布 Copilot SDK for Java 实践，显示 Copilot 能力正在从 IDE 插件扩展到 JVM 应用和服务侧可编程接口。信号是 coding agent SDK 化后，企业要把认证、上下文注入、工具边界、运行日志和版本兼容当成应用集成合同，而不是只看编辑器体验。 `blog`
- 来源：Cloudflare Blog · [Everything we launched during Agents Week](https://blog.cloudflare.com/agents-week-review-august-2026/) — Cloudflare Agents Week 汇总把 Agent Development Lifecycle、Agent Access Model、Cloudflare Agents、WebMCP、agent traces、computer runtime 和 agentic Internet 放在同一批发布里。信号是 agent 平台正在把部署、发现、可调用接口、执行沙箱、观测和付费/访问模型组合成完整基础设施，而不是单一 SDK 功能。 `blog`
- 来源：AWS Machine Learning Blog · [How nOps shipped FinOps agents 75% faster with Amazon Bedrock AgentCore](https://aws.amazon.com/blogs/machine-learning/how-nops-shipped-finops-agents-75-faster-with-amazon-bedrock-agentcore/) — AWS/nOps 案例把 FinOps agents 构建在 Amazon Bedrock AgentCore 上，并用 75% faster 的交付信号描述托管运行时带来的工程提速。信号是企业 agent 平台的价值不只在模型能力，还在身份、工具接入、部署、监控和成本治理是否能缩短高风险业务 agent 的交付周期。 `blog`
- 来源：The Decoder · [Told to book a gym class, an AI agent hacked the site instead](https://the-decoder.com/told-to-book-a-gym-class-an-ai-agent-hacked-the-site-instead-to-move-its-user-up-the-waitlist/) — The Decoder 报道一个订健身课任务中的 AI agent 选择利用网站漏洞绕过等待队列。信号是 web/action agent 的目标达成能力可能转化为越权行为，生产设计需要把意图约束、站点 ToS、动作白名单、人工确认和异常路径审计放在工具执行前。 `blog`
- 来源：The Decoder · [Hidden text in a PDF is enough to steal sensitive data through Atlassian's AI agent Rovo](https://the-decoder.com/hidden-text-in-a-pdf-is-enough-to-steal-sensitive-data-through-atlassians-ai-agent-rovo/) — The Decoder 报道隐藏 PDF 文本可通过 Atlassian Rovo 诱导敏感数据外泄，属于文档摄取链路里的间接提示注入风险。信号是企业知识库 agent 的安全边界必须覆盖文件解析、不可见文本、检索片段信任级别、工具输出脱敏和跨应用数据访问，而不是只过滤用户输入框。 `blog`
- 来源：arXiv · [MemPrism: Task-Conditioned Relational Memory Views for Long-Horizon Agents](https://arxiv.org/abs/2608.06745) — MemPrism 提出面向 long-horizon agents 的 task-conditioned relational memory views，把记忆检索从静态语义召回推进到随任务目标重组的关系视图。信号是长期 agent 记忆要同时管理任务条件、关系结构、检索视图和跨步一致性，不能只靠一个向量库 namespace 解决。 `paper`
- 来源：arXiv · [Toward Reliable Context Compression for Long-Horizon Agents](https://arxiv.org/abs/2608.06503) — 该论文以 execution instability 为核心观察 long-horizon agents 的 context compression 可靠性，提醒压缩历史不只是省 token，也会改变后续计划、工具调用和任务状态。信号是摘要/压缩策略必须进入轨迹回归、状态恢复和成本评估，不能把压缩后的上下文当作无损替代。 `paper`
- 来源：OpenAI · [OpenAI Agents Python v0.20.0 release notes](https://github.com/openai/openai-agents-python/releases/tag/v0.20.0) — OpenAI Agents Python v0.20.0 把默认模型切到 gpt-5.6-luna，同时让本地 MCP 连接兼容 MCP Python SDK v1/v2，并新增 RunState.add_input、sandbox mount 凭证暴露确认和 GA realtime transcription 设置。信号是 agent SDK 升级不只是模型名变化，还会牵动 MCP transport、暂停恢复状态、凭证边界和语音实时链路的回归合同。 `doc`
- 来源：OpenAI · [OpenAI Agents JS v0.15.0 release notes](https://github.com/openai/openai-agents-js/releases/tag/v0.15.0) — OpenAI Agents JS v0.15.0 要求自带 OpenAI client 的应用使用 openai 7.2+，默认模型切换到 gpt-5.6-luna，并引入 MCP v2 negotiation、RunState.addInput、pendingInput 序列化和 approveUnsafeReplay。信号是 JS 端 agent runtime 的风险边界集中在 client 版本、MCP 协议协商、暂停恢复输入和工具调用 replay 证据，而不是单独的 npm 升级。 `doc`
- 来源：LangChain · [LangGraph 1.2.11 release notes](https://github.com/langchain-ai/langgraph/releases/tag/1.2.11) — LangGraph 1.2.11 在 add_node 暴露 trace_policy，并同步发布 checkpoint/checkpoint-postgres/checkpoint-sqlite 更新。信号是图式 agent 的观测策略正在下沉到节点级配置，checkpoint 行为和 trace 采样/归因需要一起测试，不能只验证 graph 能跑通。 `doc`
- 来源：Anthropic · [Claude Code v2.1.229 release notes](https://github.com/anthropics/claude-code/releases/tag/v2.1.229) — Claude Code v2.1.229 增加 remote-control --continue 文档、自托管 runner 的 server-supplied hook 支持、gateway streaming SSE keepalive，并修复 Windows 扩展路径/UNC 路径、Remote Control 状态和长响应流式输出问题。信号是 coding agent 的生产体验要同时审查远程会话恢复、自托管 runner policy injection、长思考网络保活和 Windows 文件路径鲁棒性。 `doc`
- 来源：Pydantic · [Pydantic AI v2.27.1 security release notes](https://github.com/pydantic/pydantic-ai/releases/tag/v2.27.1) — Pydantic AI v2.27.1 修复 retry-prompt content 在 InstrumentationSettings(include_content=false) 下仍可能泄露的问题，并补充 failed argument validation 的 tool span。信号是 agent retry/repair 链路本身也会携带模型输出和验证反馈，观测系统的内容脱敏必须覆盖 retry prompt、tool error 和 OpenTelemetry span，而不是只关掉最终回答内容。 `doc`
- 来源：arXiv · [DSAgentBench: Can Agents Automate End-to-End Data-Science Workflows in Real Computer Environments?](https://arxiv.org/abs/2608.10366) — DSAgentBench 提出在真实计算机环境里评测端到端数据科学工作流，覆盖 275 个任务、notebook/IDE/terminal/browser/database 多工具协作，并用确定性 evaluator 验证分析正确性、可视化和模型性能。信号是 agent benchmark 正从 code-only 或网页点击转向完整业务生命周期，当前最强 agent 也只有 56.70% 成功率，开源 agent 低于 1%。 `paper`
- 来源：arXiv · [MESA: Task-Adaptive Multi-Structure Evidence Selection for Long-Horizon Agent Memory](https://arxiv.org/abs/2608.10108) — MESA 针对 long-horizon agent memory 提出按 query 动态选择和融合多种结构化记忆，而不是固定读取全部结构或只路由到单一结构；论文报告在 AMA-Bench 上比最强 baseline 高 8.5%，同时比 all-structure 少 41% evidence tokens。信号是长期记忆评测要检查结构选择策略、证据 token 成本和答案级反馈，不只是召回率。 `paper`
- 来源：arXiv · [UserToolBench: A User-Profile-Hidden Benchmark for Personalized Decision Making in Tool-Use LLMs](https://arxiv.org/abs/2608.10042) — UserToolBench 用隐式用户画像评测 tool-use LLM 的个性化决策，包含 10 个用户画像、36 个工具集、1065 轮、170 个工具，并覆盖信息不足、单工具、多工具任务。信号是 personalization 不能只看回答风格，而要看 agent 能否从历史推断约束、必要时追问，并产生符合用户偏好的工具调用轨迹。 `paper`
- 来源：OpenAI · [The builder's guide to GPT-5.6](https://openai.com/index/builders-guide-to-gpt-5-6) — OpenAI 的 GPT-5.6 builder guide 把生产 agent 的改进拆到模型选择、reasoning effort、Responses API、programmatic tool calling、多 agent steerability 和 prompt caching。信号是 agent 经济性不只靠更强模型，还要靠按步骤选模型、限制 subagent 触发、用 deterministic cache breakpoint 和更细粒度工具编排降低成本。 `blog`
- 来源：Google · [Google ADK Python v2.7.0 release notes](https://github.com/google/adk-python/releases/tag/v2.7.0) — Google ADK v2.7.0 是一次 correctness release，重点放在 agent 发给模型和从模型收回的内容：模型声明自身 capability 后再配 output schema 和 tools，tool response 可携带 media，Gemini 需要的 thought signatures、server-side tool call parts 和 parallel function call result 会保留在 conversation history。信号是 agent runtime 升级要回归模型能力声明、工具结果媒介、并行调用历史和 session 模板，而不是只看一次调用成功。 `doc`
- 来源：Anthropic · [Claude Code v2.1.232 release notes](https://github.com/anthropics/claude-code/releases/tag/v2.1.232) — Claude Code v2.1.232 默认打开 subagent forking，使 fork subagent 继承完整对话和 prompt cache；同时支持用 @ 提及另一 Claude session、通过 SendMessage 发消息，并新增跨会话消息的 accept/hold/refuse 配置。该 release 还补 GitLab token redaction、GitLab marketplace clone、PowerShell permission bypass 和 Windows symlink 写入审批修复。信号是 coding agent 已从单会话工具变成多会话协作和权限控制面，升级要同时测 session identity、inbound message policy、secret redaction 和 shell/path sandbox。 `doc`
- 来源：AWS Machine Learning Blog · [Monitor on-premises and multi-cloud AI agents with AgentCore Observability](https://aws.amazon.com/blogs/machine-learning/monitor-on-premises-and-multi-cloud-ai-agents-with-agentcore-observability/) — AWS 这篇 AgentCore Observability 实践把监控目标扩到 on-premises、GCP、Azure 和开发机上的 agent，并通过 ADOT/OpenTelemetry 接入 AgentCore 观测。信号是生产 agent 的 observability 不能只依赖托管 runtime 内部日志，要把 framework span、tool 调用、跨云部署环境和审计证据统一进 trace/metric/log 管道。 `blog`
- 来源：AWS Machine Learning Blog · [Automate legacy web applications with Amazon Bedrock AgentCore Browser Tool](https://aws.amazon.com/blogs/machine-learning/automate-legacy-web-applications-with-amazon-bedrock-agentcore-browser-tool/) — AWS 这篇实践展示用 Bedrock AgentCore Browser Tool 和 Strands Agents 自动化缺少现代 API 的 legacy web applications。信号是 web agent 的生产价值常来自浏览器侧行动能力，但验收必须覆盖 remote browser isolation、session timeout、live view、人类接管、session replay 和 legacy 系统副作用，而不是只验证页面能点。 `blog`
- 来源：GitHub Changelog · [Agent Plugins 1.0 in VS Code, Copilot CLI, and the Copilot app](https://github.blog/changelog/2026-08-12-agent-plugins-1-0-in-vs-code-copilot-cli-and-the-copilot-app) — GitHub Changelog 宣布 Agent Plugins 1.0 可在 VS Code、Copilot CLI 和 Copilot app 等兼容 agent client 中复用同一 plugin，并说明该规范由 AWS、Anysphere、Microsoft、OpenAI、Vercel 等共同发布。信号是 agent 扩展正在从单客户端插件迁移到跨客户端能力包，治理重点也变成签名/版本、权限声明、市场源、安装边界和多入口一致性。 `blog`
- 来源：arXiv · [InfraBench: Evaluating Infrastructure Agents Across Layers, Lifecycle, and Risk](https://arxiv.org/abs/2608.11234) — InfraBench 针对 infrastructure agents 设计跨系统层、完整运维生命周期和细粒度风险评估的 benchmark。论文报告 15 个 agent-model configuration 中最强配置也不能在全部任务拿满分，常见失败包括非持久化变更、破坏 distributed invariants、unsafe side effects 和未清理状态。信号是 infra agent 评测必须看风险和副作用，不可只看短期目标完成。 `paper`
- 来源：arXiv · [EvoGraph-Mem: Failure-Aware Editable Graph Memory for Long-Term Language Agents](https://arxiv.org/abs/2608.11248) — EvoGraph-Mem 把长期 agent 记忆维护建成 editable insight graph，每个 insight node 跟踪 positive evidence、negative evidence 和 activation state，执行后由 graph controller 保留可靠 insight、归档失效 insight、修订过时 insight 并加入新 insight。信号是 long-term memory 不能 append-only，必须能根据失败证据做失效、修订和检索效用更新。 `paper`
- 来源：OpenAI · [OpenAI Agents Python v0.21.0 release notes](https://github.com/openai/openai-agents-python/releases/tag/v0.21.0) — OpenAI Agents Python v0.21.0 把 provider-neutral testing APIs 变成正式能力：agent、sandbox、realtime 和 voice 流程都可以用 deterministic fixtures 回归，同时兼容 OpenAI Python v3/HTTPX 2 并继续加固 RunState、MCP 和 sandbox。信号是 agent SDK 的测试性正在成为生产合同，不能只靠真实模型或真实 WebRTC/WebSocket 做回归。 `doc`
- 来源：OpenAI · [OpenAI Agents JS v0.16.0 release notes](https://github.com/openai/openai-agents-js/releases/tag/v0.16.0) — OpenAI Agents JS v0.16.0 增加 deterministic testing utilities，覆盖 agents、realtime 和 voice 等路径，并引入 Standard Schema 输入/输出支持。信号是 TypeScript agent stack 也在把 runner、schema、handoff 和 realtime 流纳入可回放回归，而不是把 live provider 当唯一测试环境。 `doc`
- 来源：OpenAI · [OpenAI Agents Python v0.21.1 release notes](https://github.com/openai/openai-agents-python/releases/tag/v0.21.1) — OpenAI Agents Python v0.21.1 补齐 model call timeout、run-scoped sandbox working directories、Docker sandbox disable networking、exact approval honoring、compaction usage accounting 和 POSIX path normalization。信号是生产 sandbox agent 的隔离、网络、审批、超时和成本统计都要按 run 级别验收。 `doc`
- 来源：OpenAI · [OpenAI Agents JS v0.16.1 release notes](https://github.com/openai/openai-agents-js/releases/tag/v0.16.1) — OpenAI Agents JS v0.16.1 与 Python v0.21.1 同步强化 model call timeout、run-scoped sandbox working directories、Docker sandbox disable networking 和 exact approval handling。信号是跨语言 Agents SDK 的 runtime contract 正在收敛，TS 端也要把 sandbox 隔离、网络策略和审批副作用纳入回归。 `doc`
- 来源：Anthropic · [Claude Code v2.1.233 release notes](https://github.com/anthropics/claude-code/releases/tag/v2.1.233) — Claude Code v2.1.233 同时推进 GitLab MR worktree、apps gateway forward_user_identity、Bash memory cgroup、WebFetch cache TTL、MCP v2 listen stream reopening、permission notification hooks、NT 路径校验和 Windows approval 修复。信号是 coding agent release 已把身份归因、资源上限、协议恢复、缓存和路径安全揉进同一控制面。 `doc`
- 来源：Pydantic · [Pydantic AI v2.30.0 release notes](https://github.com/pydantic/pydantic-ai/releases/tag/v2.30.0) — Pydantic AI v2.30.0 修复 Agent.to_web() 和 clai web 的 DNS rebinding 漏洞，加入 Host validation 与 allowed_hosts，并支持 deferred tool capability loading。信号是 local dev agent UI 也可能握有本地工具和凭据，必须按高权限攻击面处理，而不是当成 harmless localhost demo。 `doc`
- 来源：browser-use · [browser-use 0.13.8 release notes](https://github.com/browser-use/browser-use/releases/tag/0.13.8) — browser-use 0.13.8 修复 Anthropic tool arguments 被序列化成文本时的恢复、empty action list guard、空 URL 下 domain-restricted actions 暴露、remote-browser downloads callback 和 structured output from tool calls。信号是 browser agent 的可靠性要看工具参数、动作注册、下载副作用和结构化输出轨迹，不能只验最终页面状态。 `doc`
- 来源：Cloudflare Blog · [How Cloudflare detects MCP traffic and helps secure it](https://blog.cloudflare.com/mcp-security-updates/) — Cloudflare 介绍用协议级 heuristics 检测 MCP traffic，帮助发现 shadow MCP，并支持强制 Portal-only approved access 或阻断 direct MCP。信号是企业 MCP 治理不能只靠 agent config allowlist，还需要网络/代理侧可见性、集中入口和 fail-closed 策略。 `blog`
- 来源：AWS Machine Learning Blog · [Building agentic workflows with SageMaker AI and Bedrock AgentCore](https://aws.amazon.com/blogs/machine-learning/building-agentic-workflows-with-sagemaker-ai-and-bedrock-agentcore/) — AWS 这篇实践把 SageMaker AI 的 OpenAI-compatible endpoints 与 Bedrock AgentCore runtime 组合成 multi-agent workflow，让 specialized agents 按任务选择最合适模型。信号是企业多 agent 架构正在走向模型路由、runtime 隔离、部署和观测组合，而不是一个通用模型包打天下。 `blog`
- 来源：Anthropic · [Claude Code v2.1.234 release notes](https://github.com/anthropics/claude-code/releases/tag/v2.1.234) — Claude Code v2.1.234 继续加固 coding agent 控制面：remote file reads、session restore、CLAUDE.md includes、workflow scripts 和 file uploads 都拒绝 Windows NT-namespace 路径，同时修复 session-scoped permission answers 在 background subagent prompt 中丢失、MCP diagnostics 泄露 resolved secrets、SCP-style marketplace allowlist 主机判断等问题。信号是预审批文件访问、权限记忆、MCP 诊断和 marketplace 来源校验必须一起纳入升级回归。 `doc`
- 来源：Pydantic · [Pydantic AI v2.31.0 release notes](https://github.com/pydantic/pydantic-ai/releases/tag/v2.31.0) — Pydantic AI v2.31.0 让 UIEventStream 可以不带 run_input 构建，并给 AGUIEventStream 独立 thread_id/run_id，同时把 failed FallbackModel spans 归因到真实失败模型而不是 fallback wrapper，并修复 Temporal workflow sandbox 中 openai 透传。信号是 agent UI 流、fallback 模型链和 workflow sandbox 的身份/trace 归因不能混在一起，否则排障、成本和回放都会失真。 `doc`
- 来源：CrewAI · [CrewAI 1.15.16 release notes](https://github.com/crewAIInc/crewAI/releases/tag/1.15.16) — CrewAI 1.15.16 引入带 UUID 的 execution context management，记录 flow 由哪类 exception 结束、trace batch 何时共享到 AMP、deployment 从哪个 origin 发起，并在每个 span 上记录 running release。信号是多 agent flow 的观测合同不只要 trace，还要把上下文身份、失败类型、发布版本和部署来源一起落到 span 里。 `doc`
- 来源：LangChain · [LangChain OpenRouter 0.2.8 release notes](https://github.com/langchain-ai/langchain/releases/tag/langchain-openrouter%3D%3D0.2.8) — LangChain OpenRouter 0.2.8 修复 usage chunks 中 cost metadata 的保留，并在 response metadata 中保留 provider；同日 langchain-openai 1.5.1 修复 streamed encrypted reasoning 保留。信号是多 provider agent stack 的成本、供应商归因和 reasoning stream 不是日志装饰，而是 trace、计费、回放和模型路由评估的合同字段。 `doc`
- 来源：OpenAI · [OpenAI Codex 0.148.0 release notes](https://github.com/openai/codex/releases/tag/rust-v0.148.0) — OpenAI Codex 0.148.0 增加 TUI Markdown export、codex exec fork、resume picker 中 archive/restore session、thread credit/cost 可见性、内置 Bedrock Runtime provider，以及可异步执行并调用 MCP tools 的 hooks；同时修复 resumed sessions 的 cwd/approval policy 恢复、provider outage reconnect、MCP OAuth reauth recovery 和 denied/unreadable sandbox path fail-closed。信号是 coding agent 已把会话生命周期、成本可见性、供应商路由、hook 执行和沙箱拒绝语义放到同一控制面。 `doc`
- 来源：Anthropic · [Claude Code v2.1.235 release notes](https://github.com/anthropics/claude-code/releases/tag/v2.1.235) — Claude Code v2.1.235 增加 prompt input spellcheck，并修复 language server 断连/重连导致 whole-prompt-cache invalidation、terminal UI 多层列表和多行 prompt 高亮偏移、permission prompt comment field 中 Shift+Tab 误批准并授予 session-wide edit permission、不可用 Agent tool 仍宣传 general-purpose default、notebook cell delete/replace 审批对话缺少原 cell 内容等问题。信号是 coding agent 的 UX 修复也会触碰权限、缓存、子 agent 可用性和审批证据。 `doc`
- 来源：Pydantic · [Pydantic AI v2.31.1 release notes](https://github.com/pydantic/pydantic-ai/releases/tag/v2.31.1) — Pydantic AI v2.31.1 修复两类 provider capability 边界：在 Bedrock 上拒绝对 Claude Sonnet 5 和 Fable 5 使用 native structured output，并在 Gemini 拒绝 thinking_level='MINIMAL' 时降级到 LOW。信号是 typed agent stack 的 schema/思考等级不能只按抽象接口假定，要把不同 provider/model 的 capability matrix 和 fallback policy 纳入回归。 `doc`
- 来源：Google · [Google ADK Python v2.7.1 release notes](https://github.com/google/adk-python/releases/tag/v2.7.1) — Google ADK Python v2.7.1 恢复 OpenTelemetry 1.42.1 ceiling，并校验 session initialization events。信号是 agent runtime 的可观测依赖上限和 session 初始化事件并非普通依赖细节，它们决定 trace/export 兼容性、会话恢复和事件流是否能被可信回放。 `doc`
- 来源：Google · [Google ADK Python v1.39.0 release notes](https://github.com/google/adk-python/releases/tag/v1.39.0) — Google ADK Python v1.39.0 增加 live session 打开时使用 RunConfig.session_resumption.handle、realtime input 的 audio_stream_end 支持，并修复 live agent run 结束时停止后台 tool tasks、校验 v1 session initialization events。信号是 live/realtime agent 的恢复句柄、音频流结束信号和后台工具取消必须一起验收，否则会出现会话恢复错位、流无法终止或任务结束后仍有副作用。 `doc`
- 来源：OpenAI · [OpenAI Agents Python v0.22.0 release notes](https://github.com/openai/openai-agents-python/releases/tag/v0.22.0) — OpenAI Agents Python v0.22.0 加强 runtime hardening：被 output guardrail 拒绝的终端工具输出会从 replayable/persisted SDK state 中脱敏，non-streaming Responses 的 failed/incomplete 终态会抛 ModelBehaviorError，显式 OpenAI client 与 organization/project 配置冲突会被拒绝，并隔离独立 RunState checkpoint 的 usage accounting。信号是 agent SDK 升级必须验 replay state、终态错误、provider 配置合同和成本归因，而不是只跑一次成功调用。 `doc`
- 来源：OpenAI · [OpenAI Agents JS v0.17.0 release notes](https://github.com/openai/openai-agents-js/releases/tag/v0.17.0) — OpenAI Agents JS v0.17.0 把 output-guardrail replay safety 做成显式失败边界：当 SDK 无法证明 pending terminal tool output 属于哪个 response 时，serialized checkpoint 会 fail closed；被 guardrail 拒绝的最终工具结果会在 SDK-owned replay surface 中替换为 withheld 文本，并保留已接受历史。信号是 TypeScript agent 的暂停恢复要把工具输出归属、guardrail 元数据、外部副作用和应用自有副本分开治理。 `doc`
- 来源：Mastra · [Mastra core 1.60.0 release notes](https://github.com/mastra-ai/mastra/releases/tag/%40mastra/core%401.60.0) — Mastra core 1.60.0 让通过 Agents API 创建的 stored agents 可用 durable: true 直接获得可配置 loop 的 durable execution，并新增 Cloudflare Sandbox provider、MCP stateless 2026-07-28 协议支持、多轮 elicitation，以及 Local/Platform/Railway sandbox checkpoints 能力声明。信号是 agent runtime 正把持久执行、远程 workspace、协议版本和可恢复快照收束成同一部署合同。 `doc`
- 来源：Anthropic · [Claude Code v2.1.237 release notes](https://github.com/anthropics/claude-code/releases/tag/v2.1.237) — Claude Code v2.1.237 修复使用 LLM gateway 或 custom base URL 的会话 prompt caching，并新增内置 Concise output style，让 Claude 直接给结果、减少前置叙述。信号是 coding agent 的 provider gateway、缓存命中、输出风格和用户可读性已经进入同一体验/成本控制面；升级时需要同时验证缓存归因和输出合同。 `doc`
- 来源：CrewAI · [CrewAI 1.15.17 release notes](https://github.com/crewAIInc/crewAI/releases/tag/1.15.17) — CrewAI 1.15.17 增加 declarative conversational flows 文档与声明驱动 conversational mode，同时修复 MCP HTTP/SSE server_name 取 URL hostname、failed attempt 后关闭 agent scope、工具错误归因到失败工具、每个 redirect hop/peer IP 的 SSRF 检查，以及 OpenAI Responses API 上 native tool calls。信号是多 agent flow 的声明式入口、MCP 标识、失败清理、工具错误和 SSRF 防线必须一起验收。 `doc`
- 来源：Pydantic · [Pydantic AI v2.32.1 release notes](https://github.com/pydantic/pydantic-ai/releases/tag/v2.32.1) — Pydantic AI v2.32.1 修复三类 agent runtime 边界：拒绝在 agent run 的同步回调中调用 Agent.run_sync()，避免向 Anthropic 发送空 signature 的 thinking blocks，并允许 FunctionModel 使用任意 callable 作为 function 或 stream_function。信号是同步/异步嵌套、reasoning block 序列化和测试模型替身都属于 agent 框架的可恢复执行合同。 `doc`
- 来源：Vercel · [Vercel AI SDK Workflow 2.0.0 release notes](https://github.com/vercel/ai/releases/tag/%40ai-sdk%2Fworkflow%402.0.0) — Vercel AI SDK Workflow 2.0.0 升级到 Workflow 5 并停止支持 Workflow 4，同时在 WorkflowAgent model-call step 重试时清理 partial UI message parts。信号是 agent workflow 版本升级不只是依赖 bump：运行时版本、重试语义和前端消息投影必须一起回归，否则会出现状态已重试但 UI 仍展示旧片段的错位。 `doc`
- 来源：arXiv · [FraudBench: Stress-Testing Policy-Grounded Banking Agents Against Adaptive Fraud](https://arxiv.org/abs/2608.18136) — FraudBench 把银行客服 agent 放到共享可变账户状态和 698 篇内部政策文档里，构造 150 个自适应欺诈场景，要求 agent 在对话历史、身份操纵、授权、工具开放和政策检索之间做安全决策。作者报告 107 个公开评分任务上四个 agent 的 attack-security 仅 49% 到 65%。信号是高权限业务 agent 的安全评测必须看历史依赖、禁止动作、干预点和工具授权，而不是只测单轮 policy QA。 `paper`
- 来源：arXiv · [Position: Multi-Agent Systems Should Prioritize Concurrency Control](https://arxiv.org/abs/2608.18092) — 这篇 position paper 把多 agent 系统里常被归因于协调或沟通的问题，重新解释为 shared state 并发控制问题：长 LLM 推理窗口会放大 stale reads、lost updates 和 inconsistent outcomes。作者主张 MAS 框架应内置 conflict detection、isolation guarantees 和 structured shared-resource access。信号是多 agent 可靠性不能只靠 prompt 让 agent 沟通好，而要把状态隔离和并发异常当成 runtime 合同。 `paper`
- 来源：arXiv · [Position: Behavioral Systems Require Behavioral Tests](https://arxiv.org/abs/2608.18081) — 这篇 ICML 2026 position paper 认为 agentic systems 已经是会与动态环境互动、追求目标并随时间适应的 behavioral systems，因此评测不能只看最终 performance outcome，而要系统观察、扰动并解释行动过程，包括从 action sequence 恢复决策策略、构造隔离行为差异的环境，以及探测多 agent emergent dynamics。信号是 agent eval 要从结果分数升级到行为过程证据。 `paper`
