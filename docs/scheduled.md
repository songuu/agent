...
| 本地测试 | 解析、去重、分页、映射等逻辑仍可用 | 当前网络、远端 API、PostgreSQL 写入一定成功 |
| PostgreSQL 读回 | 数据真实写入服务器内容库 | 最近一次任务由哪个 daemon 触发、前端是否已切到 Content API |
...
| 任务 | 本地验证 | 线上/远端验证 |
|------|----------|---------------|
| AI 资讯收集 | `pnpm news:test` | PostgreSQL `news_items` 行数、最新 `collected_at`、`/news/` 页面 |
| Notion 文章同步 | `pnpm notion:test` | PostgreSQL `notion_articles` source 覆盖、`/notion/` 页面 |
| Codefather 面试题同步 | `node --experimental-transform-types --test scripts/sync-codefather-interview-to-supabase.test.mts` | PostgreSQL `interview_questions` 的 `codefather-interview-*` 行、`/interview/` 页面 |
