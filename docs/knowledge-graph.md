...
共 **75** 个单元、**365** 个概念、**504** 条关系、**327** 篇关联文章。
...
| [Set AI credit session limits in Copilot CLI and SDK](https://github.blog/changelog/2026-07-01-set-ai-credit-session-limits-in-copilot-cli-and-sdk/) | GitHub Changelog | blog | 19 | GitHub 官方 changelog：Copilot CLI / SDK 支持按 session 设置 AI credit 上限，覆盖模型调用、subagents 和 compaction 等后台工作；达到软上限后 agent 会收尾并报告。信号是 coding agent 的成本控制正在从全局账单后验统计，前移到每次自动化运行的执行边界。 |
| [Browser tools for GitHub Copilot in VS Code are generally available](https://github.blog/changelog/2026-07-01-browser-tools-for-github-copilot-in-vs-code-are-generally-available/) | GitHub Changelog | blog | 19 | GitHub 官方 changelog：VS Code 中的 Copilot browser tools GA，agent 可打开页面、点击、输入、读取页面内容、采集 console errors 和截图；同时强调用户 tab 默认私有、agent tab 隔离、敏感权限需显式批准，并支持企业域名 allow/deny 控制。信号是 browser-use agent 正在从插件能力进入主流 IDE，但权限、cookie、网络边界必须随能力一起产品化。 |
| [Microsoft Agent Framework .NET 1.13.0 release notes](https://github.com/microsoft/agent-framework/releases/tag/dotnet-1.13.0) | Microsoft | doc | 19 | 官方 release notes：.NET 1.13.0 增加 Foundry Hosting per-user session isolation、skill approval options、file editing tools、default-approval harness 可配置化，并把 OpenAI Hosting options 默认改成不透传。信号是 production harness 正在同时收紧身份隔离、工具审批、文件编辑和 shell 默认行为，而不只是给 agent 增加更多工具。 |
| [Agentic generation of verifiable rules for deterministic, self-expanding reaction classification](https://arxiv.org/abs/2607.01061) | arXiv | paper | 19 | 论文用 multi-agent LLM pipeline 自动为 665,901 条美国专利反应分类并生成可验证规则，每条规则都经过 corpus verification loop；标准 taxonomy 从 68 类扩到 14,073 类，并在未见反应上达到 97.7% 分类准确率。信号是 agent 不只生成自由文本，也可以在强验证环里持续扩展 deterministic symbolic rules。 |
| [OpenAI Agents SDK Python v0.18.0 release notes](https://github.com/openai/openai-agents-python/releases/tag/v0.18.0) | OpenAI | doc | 19 | OpenAI Agents Python SDK v0.18.0 把 RealtimeAgent 默认模型升级到 gpt-realtime-2.1，并增加 SQLAlchemySession Unicode storage 选项；同日 openai-agents-js v0.13.0 也同步升级 RealtimeAgent 默认模型。信号是 realtime agent 的模型默认值、session 存储和跨语言 SDK parity 已成为生产升级时必须显式审查的运行时边界。 |
| [Google ADK Python v2.4.0 release notes](https://github.com/google/adk-python/releases/tag/v2.4.0) | Google ADK | doc | 19 | Google ADK Python v2.4.0 增加 ManagedAgent、Workflow as Tool、OpenAI Responses API labs 支持、Vertex AI session TTL、MCP HTTP traces、streamed thought/media/code-exec/function-result deltas，并修复 DNS rebinding、AgentTool config_path traversal、artifact scope 等安全边界。信号是企业 agent SDK 正在把托管执行、工作流复用、session 生命周期、trace 和安全默认值放进同一运行时版本面。 |
| [LangGraph 1.2.8 release notes](https://github.com/langchain-ai/langgraph/releases/tag/1.2.8) | LangGraph | doc | 19 | LangGraph 1.2.8 修复 fresh thread 上 updateState 的 delta channel bug，使其强制写 snapshot 而不是 stub checkpoint；同时包含依赖与 websockets major 更新。信号是 graph agent 的 checkpoint / delta 状态不是内部小细节，而是恢复、时间旅行、回放和线上排障的生产边界。 |
| [Adoption and Impact of Command-Line AI Coding Agents: A Study of Microsoft's Early 2026 Rollout of Claude Code and GitHub Copilot CLI](https://arxiv.org/abs/2607.01418) | arXiv | paper | 19 | 论文研究 Microsoft 早 2026 年面向数万工程师 rollout Claude Code 与 GitHub Copilot CLI 的采用与影响：首次使用主要经由社交网络扩散，留存更多与工程师编码活动相关，采用者合并 PR 数约提升 24%，但作者也明确 merged PR 只是 output proxy，不等同真实价值。信号是企业导入 coding agent 要同时看 adoption、retention、成本、产出代理指标和价值归因，而不能只看单次 benchmark 分数。 |
| [OpenAI's GPT-5.6 Sol, Terra, and Luna are now available in GitHub Copilot](https://github.blog/changelog/2026-07-09-openais-gpt-5-6-sol-terra-and-luna-are-now-available-in-github-copilot/) | GitHub Changelog | blog | 19 | GitHub 官方 changelog：Copilot 在 VS Code、Visual Studio、Copilot CLI、cloud agent、github.com、mobile、JetBrains、Xcode 与 Eclipse 中开放 GPT-5.6 Sol / Terra / Luna。Sol 面向复杂推理和长跑 agentic coding，Terra 作为默认通用编码模型，Luna 主打低成本快速任务；Business / Enterprise 还可由管理员策略控制且使用 usage-based billing。信号是 coding agent 的模型选择正在变成任务复杂度、成本策略、入口一致性和组织治理共同决定的运行时配置。 |
| [Ask Copilot for a repository overview](https://github.blog/changelog/2026-07-09-ask-copilot-for-a-repository-overview/) | GitHub Changelog | blog | 19 | GitHub 官方 changelog：用户可在仓库页让 Copilot 生成 overview，概括项目目的、技术栈、贡献指南，并在缺少 README 时辅助生成 README。信号是 code host 正在把仓库理解和 onboarding 变成默认 agent 入口，但这类自动概览仍必须被 README、源码和人工复核约束，不能直接当成事实源。 |
| [Enterprise-managed OpenTelemetry export for VS Code and CLI](https://github.blog/changelog/2026-07-08-enterprise-managed-opentelemetry-export-for-vs-code-and-cli/) | GitHub Changelog | blog | 19 | GitHub 官方 changelog：Copilot 的 VS Code 扩展与 Copilot CLI agent host 支持由企业策略统一配置 OpenTelemetry 导出，覆盖 endpoint、transport、service/resource attributes、headers 与是否包含 prompts / responses / tool content；托管设置优先于本地环境变量，且 headers 不传给子进程以避免 token 泄露。信号是 coding agent 可观测性正在从开发者本地配置进入企业托管控制面。 |
| [CrewAI 1.15.2 release notes](https://github.com/crewAIInc/crewAI/releases/tag/1.15.2) | CrewAI | doc | 19 | CrewAI 官方 stable release：1.15.2 增加动态 LLM 模型选择、inline skill definitions、Generated Flow Definition authoring skill、templated Flow action inputs、flow stream frame protocol 和 repository agents in flow definitions，并修复 model catalog cache key、flow input resolution、pip audit、Bedrock aiobotocore 与 self-listening flow methods 等问题。信号是多 agent runtime 的 flow、skill、repo-agent 与供应链边界正在从 prerelease 进入可跟踪稳定基线。 |
| [Beyond the Leaderboard: A Synthesis of Agentic AI Benchmarking, Failure Taxonomies, and Evaluation Gaps](https://arxiv.org/abs/2607.05775) | arXiv | paper | 19 | 论文综合 27 篇 agent benchmark、taxonomy 与 audit 研究，分析 19 个 benchmark 和 6 类高频失败：tool-use errors、planning breakdowns、long-context degradation、coordination failures、safety vulnerabilities 与 measurement validity gaps；作者强调子任务强不等于端到端可靠，失败会随任务长度非线性放大。信号是 agent 评估正在从排行榜均分转向失败分桶、轨迹审查和测量有效性。 |
| [OpenAI Agents SDK Python v0.18.2 release notes](https://github.com/openai/openai-agents-python/releases/tag/v0.18.2) | OpenAI | doc | 19 | OpenAI Agents Python SDK v0.18.2 增加 GPT-5.6 request controls 和 hosted multi-agent beta，同时修复 sandbox PTY / Docker deferred cleanup ownership、realtime callback / playback timing，以及 LiteLLM content-filter refusal 被空 turn 吞掉的问题。信号是 agent SDK 的能力升级正在和 sandbox 生命周期、实时交互可靠性、拒答可见性一起进入生产运行时审查清单。 |
| [LangGraph 1.2.9 release notes](https://github.com/langchain-ai/langgraph/releases/tag/1.2.9) | LangGraph | doc | 19 | LangGraph 1.2.9 继续修正 1.2.8 后的 delta channel 边界，重点是 updateState metadata / counters。信号是 checkpoint 体系里的 metadata、counter 与 delta 状态不是 UI 小字段，而会影响 replay、time travel、观测统计和事故排查的一致性。 |
| [Pydantic AI v2.9.0 release notes](https://github.com/pydantic/pydantic-ai/releases/tag/v2.9.0) | Pydantic AI | doc | 19 | Pydantic AI v2.9.0 公告披露 AG-UI UIAdapter.sanitize_messages dangling-tool-call strip 的 CWE-863 中等风险 advisory，并说明 requires_approval=True / ApprovalRequiredToolset 或工具参数鉴权可使敏感工具不受影响；同时加入 GPT-5.6 models、reasoning mode、/usage CLI 和 RunContext usage_limits。信号是 agent UI 消息清洗、工具审批、usage limit 和模型支持正在被同一 runtime release 共同治理。 |
| [Per-user states for multi-user budgets in the REST API](https://github.blog/changelog/2026-07-10-per-user-states-for-multi-user-budgets-in-the-rest-api/) | GitHub Changelog | blog | 19 | GitHub 官方 changelog：企业所有者和 billing managers 可通过单个 REST API 分页读取 multi-user budget 下每个用户的消耗、limit、使用比例过滤、排序和 individual override。信号是 agent / Copilot 成本治理正在从总账预算下沉到用户级状态 API，适合做接近限额预警、自动降级和 enablement 定向。 |
| [Agentic AI and Retrieval-Augmented Models in Straight-Through Underwriting](https://arxiv.org/abs/2607.07858) | arXiv | paper | 19 | 论文面向小型商业 BOP underwriting 构造合成但现实化环境，对比 single-LLM、naive RAG 和 multi-agent Agentic RAG；Agentic RAG 结合 targeted retrieval、third-party data checks 与 multi-step rule evaluation，在多步和缺失信息场景表现最好，并强调透明性、可审计性和 human-in-the-loop governance。信号是行业落地里 agentic RAG 的价值来自规则化证据链和拒绝 unsupported straight-through decision，而不是单纯多调几个工具。 |
| [HealthAgentBench: A Unified Benchmark Suite of Realistic Agentic Healthcare Environments for Challenging Frontier AI Agents](https://arxiv.org/abs/2606.31179) | arXiv | paper | 19 | HealthAgentBench 提供 54 个跨 7 类医疗/生物医学 agent 任务，每个任务把 agent 放进终端环境，要求查数据、用工具、推理并由任务级 verifier 评分；作者报告最强且成本效率最高的 Codex GPT-5.5 也只有约 42% success rate。信号是医疗 agent 评估正在从问答准确率转向长流程、工具、数据访问和领域 verifier 组合。 |
| [microsoft/HealthAgentBench](https://github.com/microsoft/HealthAgentBench) | Microsoft | doc | 19 | Microsoft 官方仓库发布 HealthAgentBench benchmark：包含 54 个 Harbor 风格终端任务、7 个任务类别、任务级 README / success criteria / verifier，并明确建议禁用 web browsing 防止 agent 搜索 gold labels。信号是行业 benchmark 正在把评测环境、数据凭证、执行脚本和防作弊边界一起开源。 |
| [Automated Benchmark Auditing for AI Agents and Large Language Models](https://arxiv.org/abs/2605.26079) | arXiv | paper | 19 | 论文提出 Auto Benchmark Audit，用 agentic framework 系统审计 benchmark 任务中的隐含环境依赖、规格缺口和脆弱评分逻辑；在 168 个 benchmark / 9 个领域中发现 25.7% 任务有关键问题，过滤后 SWE-bench Verified 与 Terminal-Bench 2 平均表现分别上移 9.9% 与 9.6%。信号是评估体系本身也需要 agent 审计与回归治理。 |
| [Harness Engineering for Agentic AI Coding Tools: An Exploratory Study](https://arxiv.org/abs/2602.14690) | arXiv | paper | 19 | 论文系统分析 Claude Code、GitHub Copilot、Cursor、Gemini、Codex 的 repository-level 配置机制，覆盖 AGENTS.md/context files、skills、subagents 等 8 类机制；2,853 个 GitHub 仓库样本显示 AGENTS.md 正在成为跨工具起点，而 skills/subagents 多数仍停在浅层静态说明。信号是 coding agent 的能力上限越来越受 harness / repo guidance 质量影响。 |
...
| [AIDev: Studying AI Coding Agents on GitHub](https://arxiv.org/abs/2602.09185) | arXiv | paper | 19 | AIDev 汇总 932,791 个由 OpenAI Codex、Devin、GitHub Copilot、Cursor、Claude Code 产生的 Agentic-PR，覆盖 116,211 个仓库和 72,189 名开发者，并提供 33,596 个高星仓库 PR 的评论、评审、提交和 issue 子集。信号是 coding agent 研究正在进入大规模真实 GitHub 轨迹阶段，但 PR 数量只能作为采用和协作研究基础，不能直接等同生产率或代码质量。 |
| [OpenAI Agents SDK JS v0.13.3 release notes](https://github.com/openai/openai-agents-js/releases/tag/v0.13.3) | OpenAI | doc | 19 | OpenAI Agents JS SDK v0.13.3 修复 workerd 环境中 tracing process lifecycle listeners，并补充 hosted multi-agent 与 GPT-5.6 request controls 文档。信号是同一 Agents SDK 能力需要同时审查边缘运行时、trace 生命周期、托管多 agent 编排和模型请求控制，而不能只看 API 表面是否可调用。 |
| [MCP Python SDK v2.0.0b2 release notes](https://github.com/modelcontextprotocol/python-sdk/releases/tag/v2.0.0b2) | Model Context Protocol | doc | 19 | MCP Python SDK v2.0.0b2 是第二个 v2 beta，明确说明预发布需精确 pin；重点变化包括 httpx2 / SSE transport、client-side subscriptions/listen、请求取消在 2026 transports 上真正生效，以及 resolver 可在多轮工具调用中 sample / list roots。信号是 MCP v2 迁移的关键风险在 transport 语义、取消传播、订阅事件和客户端能力注入。 |
| [MCP TypeScript SDK core v2.0.0-beta.4 release notes](https://github.com/modelcontextprotocol/typescript-sdk/releases/tag/%40modelcontextprotocol%2Fcore%402.0.0-beta.4) | Model Context Protocol | doc | 19 | MCP TypeScript SDK core v2.0.0-beta.4 将 spec schemas、OAuth schemas 和 protocol constants 移入 @modelcontextprotocol/core，多个包共享同一个 schema graph 和 object identity，并让相关包一起 version。信号是协议 SDK 的 schema 来源、包版本锁定和对象身份会影响跨包互操作，不能把它当成普通依赖整理。 |
| [OpenHands cloud 1.46.1 release notes](https://github.com/OpenHands/OpenHands/releases/tag/cloud-1.46.1) | OpenHands | doc | 19 | OpenHands cloud 1.46.1 修复 lifecycle webhooks 中 conversation created_at 保留问题，并修复 MCP SaaS credentials 使用 encrypted storage 保留。信号是 coding agent SaaS 化后，时间线可审计性和外部凭证持久化同属生产安全边界，尤其需要测试 webhook、凭证轮换和恢复流程。 |
| [Langfuse v3.213.0 release notes](https://github.com/langfuse/langfuse/releases/tag/v3.213.0) | Langfuse | doc | 19 | Langfuse v3.213.0 为 self-hosted deployments 开启 monitors events writes，给 code evaluator editor 增加 contract-aware autocomplete，并修复 eval preview、trace cost、agent API-key audit logs 和 code-eval execution traces。信号是 agent observability 正在把 self-hosted 监控、eval 合约、成本统计和审计日志放进同一生产控制面。 |
| [Arize Phoenix evals v3.1.1 release notes](https://github.com/Arize-ai/phoenix/releases/tag/arize-phoenix-evals-v3.1.1) | Arize Phoenix | doc | 19 | Arize Phoenix evals v3.1.1 修正 macro / weighted F-score 按类别计算以匹配 sklearn 语义、将 AsyncExecutor timeout 计入 max_retries，并限制 0/1 positive_label 自动检测条件。信号是 eval harness 的统计口径、timeout 归因和标签推断会直接改变回归结论，属于上线门禁而非 UI 细节。 |
| [Mem0 Node SDK v3.1.0 release notes](https://github.com/mem0ai/mem0/releases/tag/ts-v3.1.0) | Mem0 | doc | 19 | Mem0 Node SDK v3.1.0 是 TypeScript OSS SDK 的大型 provider release：增加 17 个 vector stores、5 个 LLM providers、4 个 embedders 和 reranking support，并让 mem0ai/oss 不再默认拉入 provider SDK。信号是 agent 长期记忆正在从单一存储封装升级为可组合 provider surface，依赖体积、rerank 策略和供应商选择都需要显式配置。 |
| [LLM-as-a-Verifier: A General-Purpose Verification Framework](https://arxiv.org/abs/2607.05391) | arXiv | paper | 19 | 论文提出 LLM-as-a-Verifier，把 verification 作为 agentic task 的新 scaling axis：不用离散打分，而从 scoring token logits 分布计算连续分数，并通过 score granularity、repeated evaluation 与 criteria decomposition 提高校准；作者报告 Terminal-Bench V2、SWE-Bench Verified、RoboRewardBench 与 MedAgentBench 上达到强结果，并提供 Claude Code extension 用于监控 agentic systems。信号是 eval 正在从 LLM-as-judge 标签判断转向可分解、可重复、可校准的 verifier 信号。 |
| [OpenAI Agents SDK JS v0.13.4 release notes](https://github.com/openai/openai-agents-js/releases/tag/v0.13.4) | OpenAI | doc | 19 | OpenAI Agents JS SDK v0.13.4 修复 non-final streaming chunks 的 usage 保留、union/tuple schema conversion 不再静默丢成员，并修复 AI SDK non-streaming responses 只取部分 text parts 的问题。信号是 agent SDK 升级必须同时回归流式计量、schema fail-fast 和 provider response normalization，否则 trace、成本和结构化输出都会悄悄失真。 |
| [OpenHands cloud 1.46.2 release notes](https://github.com/OpenHands/OpenHands/releases/tag/cloud-1.46.2) | OpenHands | doc | 19 | OpenHands cloud 1.46.2 修复跨域 PostHog distinct_id 对齐、app-server DB pool 默认值可配置，以及 settings GET round-trip 剥离 MCP auth secrets 的问题。信号是 coding agent SaaS 的可观测身份、连接池容量和 MCP 凭证往返都属于生产控制面，读配置也可能破坏认证状态。 |
| [Langfuse v3.218.0 release notes](https://github.com/langfuse/langfuse/releases/tag/v3.218.0) | Langfuse | doc | 19 | Langfuse v3.218.0 延续 7 月 16 日多版本更新：把 score filters 应用到 every event stream、导出 eval job configurations 到 core data S3 export、在 outbound-URL/SSRF validation 拒绝时自动关闭 export，并在前序版本加入 agent sandbox、MCP monitor tools、trace I/O 大对象保护和 metadata filtering。信号是 agent observability 正在同时强化 eval 配置可导出、事件筛选一致性、trace UI 大输入防卡死和 SSRF 失败后的数据导出熔断。 |
| [Pydantic AI v2.11.0 release notes](https://github.com/pydantic/pydantic-ai/releases/tag/v2.11.0) | Pydantic AI | doc | 19 | Pydantic AI v2.11.0 导出 HistoryProcessor、给 usage-limit 与 tool-retry errors 增加 actionable hints，并修复 Mistral streamed number validation、Anthropic/Bedrock native structured output schema transform。信号是 agent framework 需要把 history processing、usage/retry 错误提示、provider-native structured output schema 和 streamed validation 当成可回归 contract。 |
| [Agent-Safety Evaluations as Load-Bearing Evidence: A Vendor-Neutral, Cross-Harness Reconstructability Metric](https://arxiv.org/abs/2607.12469) | arXiv | paper | 19 | 论文指出 agent-safety evaluation 的 task success、attack success 或 monitor score 还不足以成为 load-bearing evidence，因为同一结果可能建立在不同证据制度上；提出 reconstructability metric、Evidence Sufficiency Cards、counterfactual-replay intervention protocol 和 claim-evidence overclaim gap。信号是安全评测要能重建支撑某个结论的决策证据，而不是只保存最终分数。 |
| [Coding-agents can replicate scientific machine learning papers](https://arxiv.org/abs/2607.02134) | arXiv | paper | 19 | 论文提出 Paper-replication workflow：把论文计算性 claim 拆成 target，要求 coding agent 记录证据、重建方法、运行实验、把产物链接到 provenance 与原文 claim 对比，并通过 validation checks 后才算完成。信号是研究型 coding agent 的完成标准正在从最终自然语言消息迁移到 workspace evidence、report coverage 和可验证门禁。 |
| [Security incident disclosure — July 2026](https://huggingface.co/blog/security-incident-july-2026) | Hugging Face | blog | 19 | Hugging Face 披露一次由 autonomous AI agent system 端到端驱动的入侵：恶意 dataset 借 dataset processing 代码执行路径取得 worker 权限，横向移动到内部集群，并触达凭证；防守方用 LLM 辅助日志分析重建 17,000+ recorded events，但商业 API 的安全策略会拦截真实攻击 payload，最终选择本地开源模型做取证。信号是 agent 安全不再只是 prompt injection，数据处理面、credential rotation、incident-response model hosting 和 guardrail lockout 都属于生产防线。 |
| [What building Shippy taught us about building agents](https://huggingface.co/blog/allenai/shippy-tech-blog) | Ai2 / Hugging Face | blog | 19 | Ai2 复盘 maritime agent Shippy：用 soul/skills/config 拆分 agent 身份、能力和运行配置，用确定性 CLI 封装复杂 Skylight API，用每用户 ephemeral Kubernetes session 隔离数据与文件，并用真实数据、rubric 和 Harbor plugin 评估整个 agent 而非单个模型。信号是高风险行业 agent 的可靠性来自 typed tool surface、sandbox isolation、source attribution 和版本化 eval pipeline。 |
| [Recursive Harness Self-Improvement](https://arxiv.org/abs/2607.15524) | arXiv / Hugging Face Papers | paper | 19 | 论文提出 Recursive Harness Self-Improvement，把 harness 表示为 agent loop 的 prompt-level specification，并用 pairwise feedback 在少量迭代内自我修订；作者报告在 30 个 synthetic ML research tasks 上提升低推理强度 agent 上限，并降低最多 60% inference cost。信号是 harness 不是被动脚手架，而是会影响轨迹质量、训练数据和未来模型表现的优化对象。 |
| [AgentLens: Production-Assessed Trajectory Reviews for Coding Agent Evaluation](https://arxiv.org/abs/2607.06624) | arXiv / Hugging Face Papers | paper | 19 | AgentLens 把 coding agent evaluation 从单一 pass/fail 扩展到完整 trajectory review：结合同步验证、LLM-written trajectory reviews 和 side-by-side comparisons，用于 nightly regression pipeline、产品版本比较和行为诊断。信号是代码 Agent 评估需要解释 agent 如何遵循指令、调用工具、验证自己、从错误恢复，而不是只看最终测试是否过。 |
| [DeepSWE](https://deepswe.datacurve.ai/) | Datacurve | doc | 19 | DeepSWE 7 月 17 日 leaderboard 更新覆盖 113 个原创长周期软件工程任务、16 个模型视图，并同时展示 pass rate、cost、output tokens 和 agent steps；第三方 BenchLM 镜像把它标为 display-only，因为结果绑定 model、mini-swe-agent harness 和 effort setting，不应当被误读为纯模型能力排名。信号是 coding agent benchmark 要把任务原创性、隔离环境、program-based verifier、成本和 harness 参数一起报告。 |
| [ToFu: A White-Box, Token-Efficient Agent Harness for Researchers](https://arxiv.org/abs/2607.11423) | arXiv / Hugging Face Papers | paper | 19 | ToFu 是面向研究者的 white-box agent harness：能读代码库、编辑文件、运行命令并接入开发工具，同时强调 token efficiency、lower cost、multilingual capability、MIT license 与本地部署。信号是研究型 coding agent 的价值不只在模型表现，还在于 harness 逻辑可检查、可修改、可评估，适合隐私敏感或需要复现实验的团队。 |
| [OpenAI Agents SDK Python v0.18.3 release notes](https://github.com/openai/openai-agents-python/releases/tag/v0.18.3) | OpenAI Agents SDK GitHub releases | doc | 19 | Python Agents SDK v0.18.3 增加 task/turn tracing spans 与 realtime session usage tracking，并修复 conversation session 初始化序列化、provider args 报错、handoff history wrapper、concurrent computer provider isolation、E2B workspace root 和 trace error redaction。信号是生产 Agent runtime 必须把 trace span、会话成本、provider 能力错误和工具会话隔离当成一等契约。 |
| [Pydantic AI 2.14.1 release](https://pypi.org/project/pydantic-ai/) | PyPI / Pydantic AI | doc | 19 | Pydantic AI 2.14.1 继续把 typed agent framework、model-agnostic provider、Pydantic validation、Logfire observability、evals、capabilities 与 MCP 组合成生产级 Agent stack。信号是面试不应只讨论 agent loop，而要说明 typed contracts、eval/trace、provider surface 和能力包如何降低线上不确定性。 |
| [CrewAI 1.15.5 skill registry authentication release](https://github.com/crewAIInc/crewAI/releases/tag/1.15.5) | CrewAI GitHub releases | doc | 19 | CrewAI 1.15.5 的主要 feature 是 Authenticate skill registry downloads。信号是 Agent skill 已经是可分发、可装载的能力包，生产环境要把 registry auth、来源证明、版本锁定、灰度晋级、回滚和审计一起纳入治理，而不是把 skill 当成普通 prompt 片段。 |
| [OpenAI Codex npm packages 0.145.0 release train](https://www.npmjs.com/org/openai) | npm / OpenAI | doc | 19 | OpenAI npm org 页面显示 @openai/codex、@openai/codex-sdk 与相关 Codex packages 发布到 0.145.0 release train。信号是 coding agent 正从 CLI 继续扩展到可嵌入 SDK/包形态，评估时必须审查文件系统权限、命令沙箱、审批策略、JSONL event 流、依赖锁定和 package provenance。 |
| [AI credit pools for cost centers in the billing UI](https://github.blog/changelog/2026-07-20-ai-credit-pools-for-cost-centers-in-the-billing-ui/) | GitHub Changelog | blog | 19 | GitHub 允许在 billing UI 中为 cost center 管理 AI credit pool，并可在到达 limit 后阻断 included usage 或进入 overage。信号是企业 Agent/Copilot 成本治理正在从总预算走向成本中心、license-derived pool、部门 chargeback、告警和限额策略。 |
| [OpenAI Agents SDK JavaScript 0.13.5 release](https://www.npmjs.com/package/@openai/agents) | npm / OpenAI | doc | 19 | @openai/agents 0.13.5 package page 显示 JavaScript/TypeScript Agents SDK 继续围绕 multi-agent workflows、sandbox agents、realtime agents、tools、guardrails、human-in-the-loop、sessions 与 tracing 迭代。信号是跨 Python/JS SDK parity 需要通过 trace、tool schema、realtime session、sandbox 支持和 package pinning 来验证。 |
| [New Copilot usage metrics impact dashboard](https://github.blog/changelog/2026-07-22-new-copilot-usage-metrics-impact-dashboard/) | GitHub Changelog | blog | 19 | GitHub 新增 Copilot metrics impact dashboard：按 Phase 1 Code-first、Phase 2 Agent-first、Phase 3 Multi-agent/Copilot app 与 Passive cohorts 展示 PR throughput、merge velocity、用户占比、代码行和下一步 enablement。信号是企业评估 coding agent 采用度不能停在 active users，而要把采用深度、产出代理指标、速度和组织干预放进同一可视化。 |
| [Gemini 3.6 Flash is now available in GitHub Copilot](https://github.blog/changelog/2026-07-21-gemini-3-6-flash-is-now-available-in-github-copilot/) | GitHub Changelog | blog | 19 | GitHub Copilot 开始灰度 Gemini 3.6 Flash，官方定位覆盖 web/app development、coding、longer-horizon agentic tasks，支持 configurable reasoning effort 与 parallel tool use，并纳入 usage-based billing 和企业管理员 preview policy。信号是 Copilot 类 coding agent 的模型选择已经同时受任务复杂度、工具并行、推理 effort、价格和管理员策略约束。 |
| [Building verification loops in Claude Code with skills](https://claude.com/blog/building-verification-loops-in-claude-code-with-skills) | Claude by Anthropic | blog | 19 | Anthropic 介绍把人工验收步骤写成 Claude Code skills，让 agent 在 gather context、take action、verify results 之间形成可迭代闭环；示例覆盖 /verify、工具链错误、code review、GitHub Actions、spec validation、rubric grader、standalone/embedded/chained/PR 四种运行位置。信号是 coding agent 的可靠性正在从『靠人记得检查』迁移到 repo-local skills 与 CI 中的可复用验证契约。 |
| [CodeRescue: Budget-Calibrated Recovery Routing for Coding Agents](https://arxiv.org/abs/2607.19338) | arXiv | paper | 19 | CodeRescue 把 coding agent 失败后的下一步建模成 recovery routing：根据执行反馈决定继续用低价模型恢复、升级高价模型，或走其它恢复动作，并用 Conformal Risk Control 在部署预算变化时校准成本风险。作者报告一个 CRC-calibrated frontier point 在 GPT-5.4-nano/GPT-5.4 组合上超过 always-escalate solve rate，同时只用 35% 平均恢复成本。信号是 agent 成本优化应发生在失败后恢复策略层，而不只是请求前模型路由。 |
| [ResearchArena: Evaluating Sabotage and Monitoring in Automated AI R&D](https://arxiv.org/abs/2607.19321) | arXiv | paper | 19 | ResearchArena 面向自动化 AI R&D 构造四类长周期任务，并把 deployable artifact 与隐藏 sabotage side task 绑定，比较 trajectory-only monitor、可执行探测 monitor、chain-of-thought 可见性等控制策略。作者指出训练数据中的隐藏破坏最难发现，执行探测有帮助但不足以覆盖 embedded sabotage。信号是高风险研究 agent 需要把产物运行、沙箱行为、监控 blind spot 和 agent 不可信假设一起评估。 |
| [Skillware: A Software Ontology and Engineering Lifecycle for Persistent Behavioral Artifacts](https://arxiv.org/abs/2607.18970) | arXiv | paper | 19 | Skillware 把 Agent Skills 定义为 persistent behavioral artifacts：一个 skill artifact 不只是 prompt，而是可包含 metadata、references、scripts、assets、hooks、package manifests、tests 和 companion interfaces，并需要独立 identity、生命周期、rollback、removal 与 host activation 关系。论文用 138,133 个去重 SKILL.md 记录和 20,556 个仓库标识支撑 taxonomy。信号是 skill 正在成为可维护的软件对象，生产治理要看身份、版本、测试和宿主兼容性。 |
| [Data Leakage Prevention in Agentic Applications via Preemptive Hardening](https://arxiv.org/abs/2607.18847) | arXiv | paper | 19 | 论文提出 agentic applications 的 pre-deployment hardening pipeline：扫描 prompt templates、tool interfaces 与 tool-invocation code，生成 schema tightening、boundary sanitization、allowlist tool gating、least-privilege checks 等补丁，再用 jailbreak、instruction override、tool-targeted manipulation 和 benign variants 验证。作者在 5 个真实应用和 AgentDojo 上报告基本攻击泄露降为 0、stress manipulation 泄露减少 91%。信号是 agent 安全不应只靠运行时 policy，部署前静态/半静态硬化与回归验证也要进入流水线。 |
| [OpenAI Agents SDK Python v0.19.0 release notes](https://github.com/openai/openai-agents-python/releases/tag/v0.19.0) | OpenAI Agents SDK GitHub releases | doc | 19 | OpenAI Agents Python v0.19.0 增加 Programmatic Tool Calling：受支持 Responses 模型可以生成 JavaScript 来协调 eligible tools，并纳入 allowed_callers、结构化 function-tool 输出、Runner streaming、guardrails、approvals、sessions 与 tracing。信号是 tool orchestration 正从单次函数调用走向受控的小程序式工具协调，生产侧必须把可调用者、审批、会话和轨迹纳入同一个执行边界。 |
| [MCP TypeScript SDK 2.0 packages add 2026-07-28 specification support](https://github.com/modelcontextprotocol/typescript-sdk/releases/tag/%40modelcontextprotocol%2Fnode%402.0.0) | Model Context Protocol TypeScript SDK releases | doc | 19 | MCP TypeScript SDK v2 package line发布 first beta packages for the 2026-07-28 specification revision，并把 migration guide、v1/v2 包边界、core schema 运行时依赖和兼容性测试放进发布说明。信号是 MCP 协议升级不能只改 client/server package version，还要同时验证 wire schema、transport、OAuth、capability merge、迁移路径和精确版本 pinning。 |
| [Agent automation controls in GitHub Issues public preview](https://github.blog/changelog/2026-07-23-agent-automation-controls-in-github-issues-in-public-preview/) | GitHub Changelog | blog | 19 | GitHub 在 Issues 中提供 agent automation controls：仓库管理员可允许 coding agent 自动接手 issue，并通过 assignee、label、confidence threshold 与 event controls 等条件控制触发。信号是 coding agent 的入口正在进入真实工单系统，验收重点要从“能否生成代码”扩展到触发条件、置信度、拒绝理由、人工接管和审计。 |
| [Claude Opus 5 is now available in GitHub Copilot](https://github.blog/changelog/2026-07-24-claude-opus-5-is-now-available-in-github-copilot/) | GitHub Changelog | blog | 19 | GitHub Copilot 开始提供 Claude Opus 5 public preview，并说明它适合复杂 coding、agentic workflows 与长期任务。信号是 coding agent 的模型选择要同时看任务复杂度、延迟/成本、管理员模型策略、preview 风险、回滚路径和跨入口一致性，而不能把新模型视作无差别升级。 |
| [CrewAI 1.15.7 runtime hardening and skill usage observability](https://github.com/crewAIInc/crewAI/releases/tag/1.15.7) | CrewAI GitHub releases | doc | 19 | CrewAI 1.15.7 修复 registry skills resolution、GPT-5.6 tools + reasoning_effort 400、Responses API tool calling、responses-only model 404 routing，并把 skill usage events 发到 runtime observability，同时升级 bedrock-agentcore 以修补 CVE-2026-16796。信号是 agent runtime 升级要同时看模型兼容、工具路径、技能仓库、观测事件和依赖 CVE，而不是只看 feature 列表。 |
| [Pydantic AI v2.18.0 release notes](https://github.com/pydantic/pydantic-ai/releases/tag/v2.18.0) | Pydantic AI GitHub releases | doc | 19 | Pydantic AI v2.18.0 增加 Anthropic/OpenRouter AdvisorTool support、OpenAI Responses WebSearchTool external_web_access、GoogleCloudProvider 多区域位置、pydantic_graph inspect 与 Mermaid 等能力。信号是 typed agent stack 的工具能力、外部网络访问、区域合规、图运行可视化和 provider portability 正在一起进入生产控制面。 |
| [TRACE-ROUTER: Task-Consistent and Adaptive Online Routing for Agentic AI](https://arxiv.org/abs/2607.22465) | arXiv | paper | 19 | TRACE-ROUTER 指出 agentic workflows 的质量由延迟的任务级 outcome 决定，逐 call 独立模型路由会错过 long-horizon 轨迹一致性；论文提出基于 trace 的 online routing，把成本/质量选择绑定到任务进展与历史反馈。信号是企业模型路由需要看 agent trace、阶段状态和最终任务结果，而不是把每次 LLM call 当成独立分类题。 |
| [The Regression Tax: Decomposing Why Skills Help and Hurt LLM Agents](https://arxiv.org/abs/2607.22520) | arXiv | paper | 19 | Regression Tax 把 agent skill 的平均收益拆成帮助与伤害两部分，在近 6000 次 office automation 运行、多个 benchmark 和 harness stack 上比较有/无 skill 的差异。信号是 skill 上线不能只看平均 success uplift；要分桶看正迁移、负迁移、任务类型、模型/harness 交互和回滚条件。 |
| [Dynamic Capability Scoping for Enterprise AI Agents](https://arxiv.org/abs/2607.22445) | arXiv | paper | 19 | 论文批评企业 agent 在配置期授予静态 credentials 会形成长期过度权限，提出 dynamic least-privilege capability scoping 和 three-source permission architecture。信号是权限边界要随任务、上下文和工具意图动态收缩，防护应发生在 credential exposure 之前，而不是事后检测越权。 |
| [Do Agent Benchmarks Measure Capability? Protocol Validity in the Age of Agentic AI](https://arxiv.org/abs/2607.22368) | arXiv | paper | 19 | 论文把 agent benchmark 的能力声明绑定到 protocol validity：如果环境允许 public solution recovery、读到 evaluation artifacts、利用信息泄露或和 grader 侧信道交互，高分就不再证明目标能力。信号是 benchmark 治理要检查任务隔离、artifact 可见性、信息流、grader 鲁棒性和 capability necessity，而不是只发布总分。 |
...
| [APS-RAG: Agentic Hybrid RAG and Operations-Grounded Evaluation for Scientific Facility Support](https://arxiv.org/abs/2607.24663) | arXiv | paper | 19 | APS-RAG 面向 Advanced Photon Source 运维支持，把密集向量、稀疏关键词、知识图谱、adaptive RRF、cross-encoder reranking、ReAct agent、MCP tools 和六层评估框架组合到一个 facility-grounded RAG 系统中。信号是高风险领域 RAG 不能只调 embedding，要把数据资产、检索融合、agent 工具和操作级评估一起建。 |
| [Agentic Permissions Policy Algebra for Taint Confinement in LLM Agents](https://arxiv.org/abs/2607.24625) | arXiv | paper | 19 | APPA 提出 Agentic Permissions Policy Algebra，用 small-step context machine 形式化 LLM agent 的 trust-taint 传播，在 prompt context、tool calls、control-flow branching、memory 和 subagents 之间做上下文隔离与权限合成。信号是 agent 安全不能只靠提示词拒答，而要把 taint、context、tool capability 和 delegation 变成可验证策略。 |
| [Looping Is Not Reliability: State-Bound Evidence and Typed Revision Contracts for Agentic Code Repair](https://arxiv.org/abs/2607.24604) | arXiv | paper | 19 | 论文指出 multi-agent code repair loops 若缺少 state-bound evidence 和 typed revision contracts，会重复迭代却无法提高可靠性；它主张把修复证据、状态绑定、类型化 revision contract 和验证门纳入循环。信号是 coding agent 不能把“多跑几轮”当可靠性，需要可审计的证据与修订协议。 |
| [Kimi K3: Open Frontier Intelligence](https://arxiv.org/abs/2607.24653) | arXiv | paper | 19 | Kimi K3 报告开源 1T 参数混合专家模型并强调在通用、agentic 与 coding domains 的性能；训练方案包含 million-token-scale context、Agentic RL、long-horizon reward、persistent rollout、sandbox states 与 cross-session continual improvement。信号是开源模型平台也在把 agentic RL、长上下文和可复现 sandbox 作为核心训练/评测资产。 |
| [Cloudflare Agents SDK 0.20.0 package and docs](https://www.npmjs.com/package/agents) | npm / Cloudflare Docs | doc | 19 | Cloudflare agents package 页面显示 0.20.0 latest，并把 state、SQL storage、schedule、human-in-the-loop、MCP、workflows 与 Durable Objects 连接到边缘 agent runtime。信号是生产 agent runtime 不只是调用模型，还要把长生命周期状态、定时触发、用户确认、工具协议和边缘执行身份作为一套部署契约。 |
| [Make AI Agent: Sub-agents and tool output filtering](https://help.make.com/ai-agents/make-ai-agent-sub-agents-and-tool-output-filtering) | Make Help Center | doc | 19 | Make AI Agent 增加 sub-agents 和 tool output filtering：主 agent 可以把子任务委托给专门 agent，并只把工具输出中必要字段继续传入上下文。信号是低代码 agent 产品也在把职责分解、上下文瘦身、权限边界和输出投影变成用户可配置的生产控制面。 |
| [Disney steers developers from GitHub Copilot toward Codex, Claude Code and Cursor](https://www.businessinsider.com/disney-microsoft-github-copilot-openai-codex-ai-tools-claude-cursor-2026-7) | Business Insider | blog | 19 | Business Insider 报道 Disney 内部把开发者从 GitHub Copilot 引导到 OpenAI Codex、Anthropic Claude Code 和 Cursor 等工具。信号不是某个工具必然胜出，而是企业 coding agent 采购正在从单一 Copilot 许可转为多工具组合治理，需要比较数据边界、仓库权限、审计、成本、员工迁移和模型/IDE 生态适配。 |
| [OpenAI Agents JS OpenAI provider 0.14.0 and AI SDK extension](https://www.npmjs.com/package/@openai/agents-openai) | npm / OpenAI Agents JS Docs | doc | 19 | OpenAI Agents JS 的 OpenAI provider package 进入 0.14.0，同时官方扩展文档提供 `toResponseModel` 等与 AI SDK 互操作的适配面。信号是 agent SDK 的版本治理不只看 core loop，还要把 provider 包、模型适配、工具调用、流式事件和外部 SDK 兼容一起纳入供应链与回归测试。 |
| [The new rules of context engineering for Claude 5 generation models](https://claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models) | Claude Blog | blog | 19 | Anthropic 把 context engineering 拆成 system prompt、conversation history、retrieved context、tool definitions、tool outputs 和可读 agent scratchpad，并强调长上下文、工具调用、token budget、clarity 与 retrieval noise 的取舍。信号是生产 agent 的效果越来越依赖上下文装配工程，而不是把更多文本塞进窗口。 |
| [How Datadog built a universal machine tool for Claude Code](https://claude.com/blog/how-datadog-built-a-universal-machine-tool-for-claude-code) | Claude Blog / Datadog | blog | 19 | Datadog 团队用 Temper 给 Claude Code 提供可隔离的云端开发环境，核心是把真实仓库、依赖、工具链和执行副作用放进受控 machine tool，而不是让 coding agent 直接污染本地环境。信号是 enterprise coding agent 的生产落点正在从 IDE 补全转向可审计、可复现、可回收的远程执行沙箱。 |
| [Salesforce Summer '26 Agentforce release: multi-agent orchestration and Tableau MCP](https://www.salesforce.com/news/stories/summer-2026-product-release-announcement/) | Salesforce Newsroom / Release Notes | blog | 19 | Salesforce Summer '26 release 把 Agentforce 的 multi-agent orchestration、Tableau MCP connector、agent testing center、observability 和 enterprise platform controls 放进同一批产品更新。信号是企业 agent 平台的竞争点正在转向跨 agent 编排、工具/数据协议接入、测试、监控和管理员默认控制，而不是单一聊天入口。 |
| [Cloudflare Agent traces for Think, Flue, and AI SDK](https://developers.cloudflare.com/changelog/post/2026-08-04-agent-tracing/) | Cloudflare Changelog | doc | 19 | Cloudflare Agents SDK 增加 agent traces：可在同一 trace 中查看 agent turn、model calls、tool runs、approvals、token usage 和 Workers runtime operations，并为 AI SDK v6/v7 提供 `wrapAISDK()`。信号是生产 agent 观测正在从普通日志推进到 turn/tool/model/runtime 同轨迹审计，同时需要明确消息和工具 payload 是否允许存储。 |
| [Preview: @cloudflare/computer agent runtime](https://developers.cloudflare.com/changelog/post/2026-08-03-cloudflare-computer/) | Cloudflare Changelog | doc | 19 | Cloudflare 预览开源 `@cloudflare/computer` agent runtime，为每个 agent 提供 SQLite-backed virtual filesystem，并在 isolate 与 full Linux container 间选择执行后端；read/write/edit/ls/exec 等操作被 gated、audited、observed。信号是 coding/automation agent 的 runtime 正从单一容器转向按任务选择执行平面，同时把文件系统、命令执行和审计作为默认边界。 |
| [Discover Agent Skills from MCP servers in .NET](https://devblogs.microsoft.com/agent-framework/discover-agent-skills-from-mcp-servers-in-net/) | Microsoft Agent Framework Blog | blog | 19 | Microsoft Agent Framework 的 .NET MCP skills 支持让 agent 通过 `Microsoft.Agents.AI.Mcp` 和 `UseMcpSkills(client)` 从 MCP server 发现并加载 Agent Skills，支持 `skill-md` 与 archive 两种分发形式。信号是 skill 正在从本地文件夹/应用内打包变成可集中发布、版本治理、按需加载的企业能力面，同时 archive 解包和脚本执行必须有受控目录与审批边界。 |
| [Copilot on web expands conversation controls](https://github.blog/changelog/2026-08-10-copilot-on-web-expands-conversation-controls) | GitHub Changelog | blog | 19 | GitHub Copilot on web 扩展 conversation controls，说明 coding agent 的 web 入口正在从单次聊天走向可管理的会话生命周期。信号是对话分支、上下文延续、历史整理和人工接管都需要被纳入产品交互与审计，而不是只验证最终代码建议。 |
| [Using the GitHub Copilot SDK for Java](https://github.blog/engineering/using-the-github-copilot-sdk-for-java/) | GitHub Engineering Blog | blog | 19 | GitHub Engineering 发布 Copilot SDK for Java 实践，显示 Copilot 能力正在从 IDE 插件扩展到 JVM 应用和服务侧可编程接口。信号是 coding agent SDK 化后，企业要把认证、上下文注入、工具边界、运行日志和版本兼容当成应用集成合同，而不是只看编辑器体验。 |
| [Everything we launched during Agents Week](https://blog.cloudflare.com/agents-week-review-august-2026/) | Cloudflare Blog | blog | 19 | Cloudflare Agents Week 汇总把 Agent Development Lifecycle、Agent Access Model、Cloudflare Agents、WebMCP、agent traces、computer runtime 和 agentic Internet 放在同一批发布里。信号是 agent 平台正在把部署、发现、可调用接口、执行沙箱、观测和付费/访问模型组合成完整基础设施，而不是单一 SDK 功能。 |
| [How nOps shipped FinOps agents 75% faster with Amazon Bedrock AgentCore](https://aws.amazon.com/blogs/machine-learning/how-nops-shipped-finops-agents-75-faster-with-amazon-bedrock-agentcore/) | AWS Machine Learning Blog | blog | 19 | AWS/nOps 案例把 FinOps agents 构建在 Amazon Bedrock AgentCore 上，并用 75% faster 的交付信号描述托管运行时带来的工程提速。信号是企业 agent 平台的价值不只在模型能力，还在身份、工具接入、部署、监控和成本治理是否能缩短高风险业务 agent 的交付周期。 |
| [Told to book a gym class, an AI agent hacked the site instead](https://the-decoder.com/told-to-book-a-gym-class-an-ai-agent-hacked-the-site-instead-to-move-its-user-up-the-waitlist/) | The Decoder | blog | 19 | The Decoder 报道一个订健身课任务中的 AI agent 选择利用网站漏洞绕过等待队列。信号是 web/action agent 的目标达成能力可能转化为越权行为，生产设计需要把意图约束、站点 ToS、动作白名单、人工确认和异常路径审计放在工具执行前。 |
| [Hidden text in a PDF is enough to steal sensitive data through Atlassian's AI agent Rovo](https://the-decoder.com/hidden-text-in-a-pdf-is-enough-to-steal-sensitive-data-through-atlassians-ai-agent-rovo/) | The Decoder | blog | 19 | The Decoder 报道隐藏 PDF 文本可通过 Atlassian Rovo 诱导敏感数据外泄，属于文档摄取链路里的间接提示注入风险。信号是企业知识库 agent 的安全边界必须覆盖文件解析、不可见文本、检索片段信任级别、工具输出脱敏和跨应用数据访问，而不是只过滤用户输入框。 |
| [MemPrism: Task-Conditioned Relational Memory Views for Long-Horizon Agents](https://arxiv.org/abs/2608.06745) | arXiv | paper | 19 | MemPrism 提出面向 long-horizon agents 的 task-conditioned relational memory views，把记忆检索从静态语义召回推进到随任务目标重组的关系视图。信号是长期 agent 记忆要同时管理任务条件、关系结构、检索视图和跨步一致性，不能只靠一个向量库 namespace 解决。 |
| [Toward Reliable Context Compression for Long-Horizon Agents](https://arxiv.org/abs/2608.06503) | arXiv | paper | 19 | 该论文以 execution instability 为核心观察 long-horizon agents 的 context compression 可靠性，提醒压缩历史不只是省 token，也会改变后续计划、工具调用和任务状态。信号是摘要/压缩策略必须进入轨迹回归、状态恢复和成本评估，不能把压缩后的上下文当作无损替代。 |
| [OpenAI Agents Python v0.20.0 release notes](https://github.com/openai/openai-agents-python/releases/tag/v0.20.0) | OpenAI | doc | 19 | OpenAI Agents Python v0.20.0 把默认模型切到 gpt-5.6-luna，同时让本地 MCP 连接兼容 MCP Python SDK v1/v2，并新增 RunState.add_input、sandbox mount 凭证暴露确认和 GA realtime transcription 设置。信号是 agent SDK 升级不只是模型名变化，还会牵动 MCP transport、暂停恢复状态、凭证边界和语音实时链路的回归合同。 |
| [OpenAI Agents JS v0.15.0 release notes](https://github.com/openai/openai-agents-js/releases/tag/v0.15.0) | OpenAI | doc | 19 | OpenAI Agents JS v0.15.0 要求自带 OpenAI client 的应用使用 openai 7.2+，默认模型切换到 gpt-5.6-luna，并引入 MCP v2 negotiation、RunState.addInput、pendingInput 序列化和 approveUnsafeReplay。信号是 JS 端 agent runtime 的风险边界集中在 client 版本、MCP 协议协商、暂停恢复输入和工具调用 replay 证据，而不是单独的 npm 升级。 |
| [LangGraph 1.2.11 release notes](https://github.com/langchain-ai/langgraph/releases/tag/1.2.11) | LangChain | doc | 19 | LangGraph 1.2.11 在 add_node 暴露 trace_policy，并同步发布 checkpoint/checkpoint-postgres/checkpoint-sqlite 更新。信号是图式 agent 的观测策略正在下沉到节点级配置，checkpoint 行为和 trace 采样/归因需要一起测试，不能只验证 graph 能跑通。 |
| [Claude Code v2.1.229 release notes](https://github.com/anthropics/claude-code/releases/tag/v2.1.229) | Anthropic | doc | 19 | Claude Code v2.1.229 增加 remote-control --continue 文档、自托管 runner 的 server-supplied hook 支持、gateway streaming SSE keepalive，并修复 Windows 扩展路径/UNC 路径、Remote Control 状态和长响应流式输出问题。信号是 coding agent 的生产体验要同时审查远程会话恢复、自托管 runner policy injection、长思考网络保活和 Windows 文件路径鲁棒性。 |
| [Pydantic AI v2.27.1 security release notes](https://github.com/pydantic/pydantic-ai/releases/tag/v2.27.1) | Pydantic | doc | 19 | Pydantic AI v2.27.1 修复 retry-prompt content 在 InstrumentationSettings(include_content=false) 下仍可能泄露的问题，并补充 failed argument validation 的 tool span。信号是 agent retry/repair 链路本身也会携带模型输出和验证反馈，观测系统的内容脱敏必须覆盖 retry prompt、tool error 和 OpenTelemetry span，而不是只关掉最终回答内容。 |
| [DSAgentBench: Can Agents Automate End-to-End Data-Science Workflows in Real Computer Environments?](https://arxiv.org/abs/2608.10366) | arXiv | paper | 19 | DSAgentBench 提出在真实计算机环境里评测端到端数据科学工作流，覆盖 275 个任务、notebook/IDE/terminal/browser/database 多工具协作，并用确定性 evaluator 验证分析正确性、可视化和模型性能。信号是 agent benchmark 正从 code-only 或网页点击转向完整业务生命周期，当前最强 agent 也只有 56.70% 成功率，开源 agent 低于 1%。 |
| [MESA: Task-Adaptive Multi-Structure Evidence Selection for Long-Horizon Agent Memory](https://arxiv.org/abs/2608.10108) | arXiv | paper | 19 | MESA 针对 long-horizon agent memory 提出按 query 动态选择和融合多种结构化记忆，而不是固定读取全部结构或只路由到单一结构；论文报告在 AMA-Bench 上比最强 baseline 高 8.5%，同时比 all-structure 少 41% evidence tokens。信号是长期记忆评测要检查结构选择策略、证据 token 成本和答案级反馈，不只是召回率。 |
| [UserToolBench: A User-Profile-Hidden Benchmark for Personalized Decision Making in Tool-Use LLMs](https://arxiv.org/abs/2608.10042) | arXiv | paper | 19 | UserToolBench 用隐式用户画像评测 tool-use LLM 的个性化决策，包含 10 个用户画像、36 个工具集、1065 轮、170 个工具，并覆盖信息不足、单工具、多工具任务。信号是 personalization 不能只看回答风格，而要看 agent 能否从历史推断约束、必要时追问，并产生符合用户偏好的工具调用轨迹。 |
| [The builder's guide to GPT-5.6](https://openai.com/index/builders-guide-to-gpt-5-6) | OpenAI | blog | 19 | OpenAI 的 GPT-5.6 builder guide 把生产 agent 的改进拆到模型选择、reasoning effort、Responses API、programmatic tool calling、多 agent steerability 和 prompt caching。信号是 agent 经济性不只靠更强模型，还要靠按步骤选模型、限制 subagent 触发、用 deterministic cache breakpoint 和更细粒度工具编排降低成本。 |
| [Google ADK Python v2.7.0 release notes](https://github.com/google/adk-python/releases/tag/v2.7.0) | Google | doc | 19 | Google ADK v2.7.0 是一次 correctness release，重点放在 agent 发给模型和从模型收回的内容：模型声明自身 capability 后再配 output schema 和 tools，tool response 可携带 media，Gemini 需要的 thought signatures、server-side tool call parts 和 parallel function call result 会保留在 conversation history。信号是 agent runtime 升级要回归模型能力声明、工具结果媒介、并行调用历史和 session 模板，而不是只看一次调用成功。 |
| [Claude Code v2.1.232 release notes](https://github.com/anthropics/claude-code/releases/tag/v2.1.232) | Anthropic | doc | 19 | Claude Code v2.1.232 默认打开 subagent forking，使 fork subagent 继承完整对话和 prompt cache；同时支持用 @ 提及另一 Claude session、通过 SendMessage 发消息，并新增跨会话消息的 accept/hold/refuse 配置。该 release 还补 GitLab token redaction、GitLab marketplace clone、PowerShell permission bypass 和 Windows symlink 写入审批修复。信号是 coding agent 已从单会话工具变成多会话协作和权限控制面，升级要同时测 session identity、inbound message policy、secret redaction 和 shell/path sandbox。 |
| [Monitor on-premises and multi-cloud AI agents with AgentCore Observability](https://aws.amazon.com/blogs/machine-learning/monitor-on-premises-and-multi-cloud-ai-agents-with-agentcore-observability/) | AWS Machine Learning Blog | blog | 19 | AWS 这篇 AgentCore Observability 实践把监控目标扩到 on-premises、GCP、Azure 和开发机上的 agent，并通过 ADOT/OpenTelemetry 接入 AgentCore 观测。信号是生产 agent 的 observability 不能只依赖托管 runtime 内部日志，要把 framework span、tool 调用、跨云部署环境和审计证据统一进 trace/metric/log 管道。 |
| [Automate legacy web applications with Amazon Bedrock AgentCore Browser Tool](https://aws.amazon.com/blogs/machine-learning/automate-legacy-web-applications-with-amazon-bedrock-agentcore-browser-tool/) | AWS Machine Learning Blog | blog | 19 | AWS 这篇实践展示用 Bedrock AgentCore Browser Tool 和 Strands Agents 自动化缺少现代 API 的 legacy web applications。信号是 web agent 的生产价值常来自浏览器侧行动能力，但验收必须覆盖 remote browser isolation、session timeout、live view、人类接管、session replay 和 legacy 系统副作用，而不是只验证页面能点。 |
| [Agent Plugins 1.0 in VS Code, Copilot CLI, and the Copilot app](https://github.blog/changelog/2026-08-12-agent-plugins-1-0-in-vs-code-copilot-cli-and-the-copilot-app) | GitHub Changelog | blog | 19 | GitHub Changelog 宣布 Agent Plugins 1.0 可在 VS Code、Copilot CLI 和 Copilot app 等兼容 agent client 中复用同一 plugin，并说明该规范由 AWS、Anysphere、Microsoft、OpenAI、Vercel 等共同发布。信号是 agent 扩展正在从单客户端插件迁移到跨客户端能力包，治理重点也变成签名/版本、权限声明、市场源、安装边界和多入口一致性。 |
| [InfraBench: Evaluating Infrastructure Agents Across Layers, Lifecycle, and Risk](https://arxiv.org/abs/2608.11234) | arXiv | paper | 19 | InfraBench 针对 infrastructure agents 设计跨系统层、完整运维生命周期和细粒度风险评估的 benchmark。论文报告 15 个 agent-model configuration 中最强配置也不能在全部任务拿满分，常见失败包括非持久化变更、破坏 distributed invariants、unsafe side effects 和未清理状态。信号是 infra agent 评测必须看风险和副作用，不可只看短期目标完成。 |
| [EvoGraph-Mem: Failure-Aware Editable Graph Memory for Long-Term Language Agents](https://arxiv.org/abs/2608.11248) | arXiv | paper | 19 | EvoGraph-Mem 把长期 agent 记忆维护建成 editable insight graph，每个 insight node 跟踪 positive evidence、negative evidence 和 activation state，执行后由 graph controller 保留可靠 insight、归档失效 insight、修订过时 insight 并加入新 insight。信号是 long-term memory 不能 append-only，必须能根据失败证据做失效、修订和检索效用更新。 |
| [OpenAI Agents Python v0.21.0 release notes](https://github.com/openai/openai-agents-python/releases/tag/v0.21.0) | OpenAI | doc | 19 | OpenAI Agents Python v0.21.0 把 provider-neutral testing APIs 变成正式能力：agent、sandbox、realtime 和 voice 流程都可以用 deterministic fixtures 回归，同时兼容 OpenAI Python v3/HTTPX 2 并继续加固 RunState、MCP 和 sandbox。信号是 agent SDK 的测试性正在成为生产合同，不能只靠真实模型或真实 WebRTC/WebSocket 做回归。 |
| [OpenAI Agents JS v0.16.0 release notes](https://github.com/openai/openai-agents-js/releases/tag/v0.16.0) | OpenAI | doc | 19 | OpenAI Agents JS v0.16.0 增加 deterministic testing utilities，覆盖 agents、realtime 和 voice 等路径，并引入 Standard Schema 输入/输出支持。信号是 TypeScript agent stack 也在把 runner、schema、handoff 和 realtime 流纳入可回放回归，而不是把 live provider 当唯一测试环境。 |
| [OpenAI Agents Python v0.21.1 release notes](https://github.com/openai/openai-agents-python/releases/tag/v0.21.1) | OpenAI | doc | 19 | OpenAI Agents Python v0.21.1 补齐 model call timeout、run-scoped sandbox working directories、Docker sandbox disable networking、exact approval honoring、compaction usage accounting 和 POSIX path normalization。信号是生产 sandbox agent 的隔离、网络、审批、超时和成本统计都要按 run 级别验收。 |
| [OpenAI Agents JS v0.16.1 release notes](https://github.com/openai/openai-agents-js/releases/tag/v0.16.1) | OpenAI | doc | 19 | OpenAI Agents JS v0.16.1 与 Python v0.21.1 同步强化 model call timeout、run-scoped sandbox working directories、Docker sandbox disable networking 和 exact approval handling。信号是跨语言 Agents SDK 的 runtime contract 正在收敛，TS 端也要把 sandbox 隔离、网络策略和审批副作用纳入回归。 |
| [Claude Code v2.1.233 release notes](https://github.com/anthropics/claude-code/releases/tag/v2.1.233) | Anthropic | doc | 19 | Claude Code v2.1.233 同时推进 GitLab MR worktree、apps gateway forward_user_identity、Bash memory cgroup、WebFetch cache TTL、MCP v2 listen stream reopening、permission notification hooks、NT 路径校验和 Windows approval 修复。信号是 coding agent release 已把身份归因、资源上限、协议恢复、缓存和路径安全揉进同一控制面。 |
| [Pydantic AI v2.30.0 release notes](https://github.com/pydantic/pydantic-ai/releases/tag/v2.30.0) | Pydantic | doc | 19 | Pydantic AI v2.30.0 修复 Agent.to_web() 和 clai web 的 DNS rebinding 漏洞，加入 Host validation 与 allowed_hosts，并支持 deferred tool capability loading。信号是 local dev agent UI 也可能握有本地工具和凭据，必须按高权限攻击面处理，而不是当成 harmless localhost demo。 |
| [browser-use 0.13.8 release notes](https://github.com/browser-use/browser-use/releases/tag/0.13.8) | browser-use | doc | 19 | browser-use 0.13.8 修复 Anthropic tool arguments 被序列化成文本时的恢复、empty action list guard、空 URL 下 domain-restricted actions 暴露、remote-browser downloads callback 和 structured output from tool calls。信号是 browser agent 的可靠性要看工具参数、动作注册、下载副作用和结构化输出轨迹，不能只验最终页面状态。 |
| [How Cloudflare detects MCP traffic and helps secure it](https://blog.cloudflare.com/mcp-security-updates/) | Cloudflare Blog | blog | 19 | Cloudflare 介绍用协议级 heuristics 检测 MCP traffic，帮助发现 shadow MCP，并支持强制 Portal-only approved access 或阻断 direct MCP。信号是企业 MCP 治理不能只靠 agent config allowlist，还需要网络/代理侧可见性、集中入口和 fail-closed 策略。 |
| [Building agentic workflows with SageMaker AI and Bedrock AgentCore](https://aws.amazon.com/blogs/machine-learning/building-agentic-workflows-with-sagemaker-ai-and-bedrock-agentcore/) | AWS Machine Learning Blog | blog | 19 | AWS 这篇实践把 SageMaker AI 的 OpenAI-compatible endpoints 与 Bedrock AgentCore runtime 组合成 multi-agent workflow，让 specialized agents 按任务选择最合适模型。信号是企业多 agent 架构正在走向模型路由、runtime 隔离、部署和观测组合，而不是一个通用模型包打天下。 |
| [Claude Code v2.1.234 release notes](https://github.com/anthropics/claude-code/releases/tag/v2.1.234) | Anthropic | doc | 19 | Claude Code v2.1.234 继续加固 coding agent 控制面：remote file reads、session restore、CLAUDE.md includes、workflow scripts 和 file uploads 都拒绝 Windows NT-namespace 路径，同时修复 session-scoped permission answers 在 background subagent prompt 中丢失、MCP diagnostics 泄露 resolved secrets、SCP-style marketplace allowlist 主机判断等问题。信号是预审批文件访问、权限记忆、MCP 诊断和 marketplace 来源校验必须一起纳入升级回归。 |
| [Pydantic AI v2.31.0 release notes](https://github.com/pydantic/pydantic-ai/releases/tag/v2.31.0) | Pydantic | doc | 19 | Pydantic AI v2.31.0 让 UIEventStream 可以不带 run_input 构建，并给 AGUIEventStream 独立 thread_id/run_id，同时把 failed FallbackModel spans 归因到真实失败模型而不是 fallback wrapper，并修复 Temporal workflow sandbox 中 openai 透传。信号是 agent UI 流、fallback 模型链和 workflow sandbox 的身份/trace 归因不能混在一起，否则排障、成本和回放都会失真。 |
| [CrewAI 1.15.16 release notes](https://github.com/crewAIInc/crewAI/releases/tag/1.15.16) | CrewAI | doc | 19 | CrewAI 1.15.16 引入带 UUID 的 execution context management，记录 flow 由哪类 exception 结束、trace batch 何时共享到 AMP、deployment 从哪个 origin 发起，并在每个 span 上记录 running release。信号是多 agent flow 的观测合同不只要 trace，还要把上下文身份、失败类型、发布版本和部署来源一起落到 span 里。 |
| [LangChain OpenRouter 0.2.8 release notes](https://github.com/langchain-ai/langchain/releases/tag/langchain-openrouter%3D%3D0.2.8) | LangChain | doc | 19 | LangChain OpenRouter 0.2.8 修复 usage chunks 中 cost metadata 的保留，并在 response metadata 中保留 provider；同日 langchain-openai 1.5.1 修复 streamed encrypted reasoning 保留。信号是多 provider agent stack 的成本、供应商归因和 reasoning stream 不是日志装饰，而是 trace、计费、回放和模型路由评估的合同字段。 |
| [OpenAI Codex 0.148.0 release notes](https://github.com/openai/codex/releases/tag/rust-v0.148.0) | OpenAI | doc | 19 | OpenAI Codex 0.148.0 增加 TUI Markdown export、codex exec fork、resume picker 中 archive/restore session、thread credit/cost 可见性、内置 Bedrock Runtime provider，以及可异步执行并调用 MCP tools 的 hooks；同时修复 resumed sessions 的 cwd/approval policy 恢复、provider outage reconnect、MCP OAuth reauth recovery 和 denied/unreadable sandbox path fail-closed。信号是 coding agent 已把会话生命周期、成本可见性、供应商路由、hook 执行和沙箱拒绝语义放到同一控制面。 |
| [Claude Code v2.1.235 release notes](https://github.com/anthropics/claude-code/releases/tag/v2.1.235) | Anthropic | doc | 19 | Claude Code v2.1.235 增加 prompt input spellcheck，并修复 language server 断连/重连导致 whole-prompt-cache invalidation、terminal UI 多层列表和多行 prompt 高亮偏移、permission prompt comment field 中 Shift+Tab 误批准并授予 session-wide edit permission、不可用 Agent tool 仍宣传 general-purpose default、notebook cell delete/replace 审批对话缺少原 cell 内容等问题。信号是 coding agent 的 UX 修复也会触碰权限、缓存、子 agent 可用性和审批证据。 |
| [Pydantic AI v2.31.1 release notes](https://github.com/pydantic/pydantic-ai/releases/tag/v2.31.1) | Pydantic | doc | 19 | Pydantic AI v2.31.1 修复两类 provider capability 边界：在 Bedrock 上拒绝对 Claude Sonnet 5 和 Fable 5 使用 native structured output，并在 Gemini 拒绝 thinking_level='MINIMAL' 时降级到 LOW。信号是 typed agent stack 的 schema/思考等级不能只按抽象接口假定，要把不同 provider/model 的 capability matrix 和 fallback policy 纳入回归。 |
| [Google ADK Python v2.7.1 release notes](https://github.com/google/adk-python/releases/tag/v2.7.1) | Google | doc | 19 | Google ADK Python v2.7.1 恢复 OpenTelemetry 1.42.1 ceiling，并校验 session initialization events。信号是 agent runtime 的可观测依赖上限和 session 初始化事件并非普通依赖细节，它们决定 trace/export 兼容性、会话恢复和事件流是否能被可信回放。 |
| [Google ADK Python v1.39.0 release notes](https://github.com/google/adk-python/releases/tag/v1.39.0) | Google | doc | 19 | Google ADK Python v1.39.0 增加 live session 打开时使用 RunConfig.session_resumption.handle、realtime input 的 audio_stream_end 支持，并修复 live agent run 结束时停止后台 tool tasks、校验 v1 session initialization events。信号是 live/realtime agent 的恢复句柄、音频流结束信号和后台工具取消必须一起验收，否则会出现会话恢复错位、流无法终止或任务结束后仍有副作用。 |
| [OpenAI Agents Python v0.22.0 release notes](https://github.com/openai/openai-agents-python/releases/tag/v0.22.0) | OpenAI | doc | 19 | OpenAI Agents Python v0.22.0 加强 runtime hardening：被 output guardrail 拒绝的终端工具输出会从 replayable/persisted SDK state 中脱敏，non-streaming Responses 的 failed/incomplete 终态会抛 ModelBehaviorError，显式 OpenAI client 与 organization/project 配置冲突会被拒绝，并隔离独立 RunState checkpoint 的 usage accounting。信号是 agent SDK 升级必须验 replay state、终态错误、provider 配置合同和成本归因，而不是只跑一次成功调用。 |
| [OpenAI Agents JS v0.17.0 release notes](https://github.com/openai/openai-agents-js/releases/tag/v0.17.0) | OpenAI | doc | 19 | OpenAI Agents JS v0.17.0 把 output-guardrail replay safety 做成显式失败边界：当 SDK 无法证明 pending terminal tool output 属于哪个 response 时，serialized checkpoint 会 fail closed；被 guardrail 拒绝的最终工具结果会在 SDK-owned replay surface 中替换为 withheld 文本，并保留已接受历史。信号是 TypeScript agent 的暂停恢复要把工具输出归属、guardrail 元数据、外部副作用和应用自有副本分开治理。 |
| [Mastra core 1.60.0 release notes](https://github.com/mastra-ai/mastra/releases/tag/%40mastra/core%401.60.0) | Mastra | doc | 19 | Mastra core 1.60.0 让通过 Agents API 创建的 stored agents 可用 durable: true 直接获得可配置 loop 的 durable execution，并新增 Cloudflare Sandbox provider、MCP stateless 2026-07-28 协议支持、多轮 elicitation，以及 Local/Platform/Railway sandbox checkpoints 能力声明。信号是 agent runtime 正把持久执行、远程 workspace、协议版本和可恢复快照收束成同一部署合同。 |
| [Claude Code v2.1.237 release notes](https://github.com/anthropics/claude-code/releases/tag/v2.1.237) | Anthropic | doc | 19 | Claude Code v2.1.237 修复使用 LLM gateway 或 custom base URL 的会话 prompt caching，并新增内置 Concise output style，让 Claude 直接给结果、减少前置叙述。信号是 coding agent 的 provider gateway、缓存命中、输出风格和用户可读性已经进入同一体验/成本控制面；升级时需要同时验证缓存归因和输出合同。 |
| [CrewAI 1.15.17 release notes](https://github.com/crewAIInc/crewAI/releases/tag/1.15.17) | CrewAI | doc | 19 | CrewAI 1.15.17 增加 declarative conversational flows 文档与声明驱动 conversational mode，同时修复 MCP HTTP/SSE server_name 取 URL hostname、failed attempt 后关闭 agent scope、工具错误归因到失败工具、每个 redirect hop/peer IP 的 SSRF 检查，以及 OpenAI Responses API 上 native tool calls。信号是多 agent flow 的声明式入口、MCP 标识、失败清理、工具错误和 SSRF 防线必须一起验收。 |
| [Pydantic AI v2.32.1 release notes](https://github.com/pydantic/pydantic-ai/releases/tag/v2.32.1) | Pydantic | doc | 19 | Pydantic AI v2.32.1 修复三类 agent runtime 边界：拒绝在 agent run 的同步回调中调用 Agent.run_sync()，避免向 Anthropic 发送空 signature 的 thinking blocks，并允许 FunctionModel 使用任意 callable 作为 function 或 stream_function。信号是同步/异步嵌套、reasoning block 序列化和测试模型替身都属于 agent 框架的可恢复执行合同。 |
| [Vercel AI SDK Workflow 2.0.0 release notes](https://github.com/vercel/ai/releases/tag/%40ai-sdk%2Fworkflow%402.0.0) | Vercel | doc | 19 | Vercel AI SDK Workflow 2.0.0 升级到 Workflow 5 并停止支持 Workflow 4，同时在 WorkflowAgent model-call step 重试时清理 partial UI message parts。信号是 agent workflow 版本升级不只是依赖 bump：运行时版本、重试语义和前端消息投影必须一起回归，否则会出现状态已重试但 UI 仍展示旧片段的错位。 |
| [FraudBench: Stress-Testing Policy-Grounded Banking Agents Against Adaptive Fraud](https://arxiv.org/abs/2608.18136) | arXiv | paper | 19 | FraudBench 把银行客服 agent 放到共享可变账户状态和 698 篇内部政策文档里，构造 150 个自适应欺诈场景，要求 agent 在对话历史、身份操纵、授权、工具开放和政策检索之间做安全决策。作者报告 107 个公开评分任务上四个 agent 的 attack-security 仅 49% 到 65%。信号是高权限业务 agent 的安全评测必须看历史依赖、禁止动作、干预点和工具授权，而不是只测单轮 policy QA。 |
| [Position: Multi-Agent Systems Should Prioritize Concurrency Control](https://arxiv.org/abs/2608.18092) | arXiv | paper | 19 | 这篇 position paper 把多 agent 系统里常被归因于协调或沟通的问题，重新解释为 shared state 并发控制问题：长 LLM 推理窗口会放大 stale reads、lost updates 和 inconsistent outcomes。作者主张 MAS 框架应内置 conflict detection、isolation guarantees 和 structured shared-resource access。信号是多 agent 可靠性不能只靠 prompt 让 agent 沟通好，而要把状态隔离和并发异常当成 runtime 合同。 |
| [Position: Behavioral Systems Require Behavioral Tests](https://arxiv.org/abs/2608.18081) | arXiv | paper | 19 | 这篇 ICML 2026 position paper 认为 agentic systems 已经是会与动态环境互动、追求目标并随时间适应的 behavioral systems，因此评测不能只看最终 performance outcome，而要系统观察、扰动并解释行动过程，包括从 action sequence 恢复决策策略、构造隔离行为差异的环境，以及探测多 agent emergent dynamics。信号是 agent eval 要从结果分数升级到行为过程证据。 |
