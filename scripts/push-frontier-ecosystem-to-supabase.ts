// 通过 PostgREST 把 FRONTIER_ARTICLES upsert 到自托管 Supabase。
//
...
//
// 前置：`frontier_ecosystem_articles` 表必须已存在（DDL 走 SQL Editor / 直连 Postgres，
...
async function main(): Promise<void> {
  ...
  const endpoint = `${base}/rest/v1/frontier_ecosystem_articles?on_conflict=slug`;
  ...
  // 回读核对：count
  const countResp = await fetch(`${base}/rest/v1/frontier_ecosystem_articles?select=slug`, {
    headers: {
      apikey: key,
      Authorization: `Bearer ${key}`,
