...
| L6 | [Event streaming 与前端投影](./06-event-streaming/README.md) | `values` / `updates` / `custom`、user/debug/audit 安全投影 | `npx tsx langgraph-advanced/06-event-streaming/index.ts` |
...
以下 L7-L11 暂不进入 `CHAPTERS`，因为还没有对应可运行目录。等逐章实现时，每章必须补齐 README、demo、shared 代码、smoke、知识图谱、概念可视化六件套。
...
1. 下一步做 L7 / L8：一个补长期记忆，一个补模块化边界，承接 L6 的运行时事件边界。
2. 最后做 L9-L11：这三章是上线纪律，建议每章都写回归测试，不只写讲义。
