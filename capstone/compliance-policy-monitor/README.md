# 毕业项目 · 合规政策监控 Agent

> 所属阶段：**毕业项目 · 合规治理实战**
> 预计用时：3-4 小时 | 难度：⭐⭐⭐⭐☆
> 全局导航：[课程导航](../../docs/navigation.md) · [完整大纲](../../docs/curriculum.md) · [毕业项目总览](../README.md) · [知识图谱](../../docs/knowledge-graph.md)

把政策更新、内部控制清单和业务流程映射起来，识别哪些 SOP、系统配置和培训材料需要更新。

> 离线、零 key 可设计与验证：实现时先用 fixture 和确定性规则跑通端到端闭环。真实接入时，把 fixture 替换成业务系统数据源，把规则模块替换成可配置策略或模型调用，输出契约保持不变。

## 最终交付

- [ ] 一个合规变更影响分析工作流，输出政策差异、影响流程、owner、deadline 和审计说明。
- [ ] 一组可复现 fixture，覆盖正常、边界和高风险样例。
- [ ] 一个分层 Agent 设计：输入归一、决策、工具/检索、人工确认、报告输出。
- [ ] 一套验收清单，可直接转成 smoke/eval 测试。
- [ ] 一段作品集/简历话术和面试追问准备。

## 适用角色

- 合规经理
- 内控负责人
- 流程 owner

## 核心流程

```text
导入政策版本
  -> 比较条款变化
  -> 映射控制点
  -> 识别受影响流程
  -> 分配 owner
  -> 生成变更通告
```

## 数据与接口

| 模块 | 职责 |
|------|------|
| `PolicyDiffEngine` | PolicyDiffEngine 负责本流程中的一个稳定边界，便于替换为真实 API 或数据库实现。 |
| `ControlMapper` | ControlMapper 负责本流程中的一个稳定边界，便于替换为真实 API 或数据库实现。 |
| `ProcessImpactAnalyzer` | ProcessImpactAnalyzer 负责本流程中的一个稳定边界，便于替换为真实 API 或数据库实现。 |
| `OwnerAssigner` | OwnerAssigner 负责本流程中的一个稳定边界，便于替换为真实 API 或数据库实现。 |
| `AuditNoticeBuilder` | AuditNoticeBuilder 负责本流程中的一个稳定边界，便于替换为真实 API 或数据库实现。 |

建议 fixture：

- `policy-v1.md`
- `policy-v2.md`
- `control-matrix.json`
- `process-owner-map.json`

最小输出契约：

```ts
type CapstoneResult = {
  status: "ok" | "needs_review" | "blocked";
  summary: string;
  evidence: Array<{ source: string; quote: string; confidence: "low" | "medium" | "high" }>;
  actions: Array<{ owner: string; nextStep: string; due?: string; requiresApproval: boolean }>;
  risks: Array<{ level: "low" | "medium" | "high"; reason: string }>;
};
```

## 护栏与人工确认

- 不替代法律/合规最终解释
- 影响判断必须引用条款 diff
- deadline 规则可配置
- 无 owner 的流程进入人工队列

## 里程碑

1. M0 政策 diff 和条款分类
2. M1 控制点和流程影响
3. M2 owner/deadline 通告

## 验收清单

- [ ] 新增强制条款被识别
- [ ] 删除条款不触发新增任务
- [ ] 影响流程可追溯
- [ ] 无 owner 报错
- [ ] deadline 按等级计算
- [ ] 通告含证据引用

## 可扩展方向

- 接监管 RSS
- 生成培训更新清单
- 推送 GRC 系统
- 按地区维护政策包

## 如何写进简历

> 实现合规政策监控 Agent：对比政策版本，映射控制矩阵和业务流程，生成 owner/deadline 明确的影响任务与审计说明。

## 面试追问

1. 政策 diff 为什么要映射到流程？
2. 无 owner 的控制点怎么处理？
3. 合规 Agent 的责任边界是什么？
4. 如何避免把政策解释自动化过头？

<!-- KG:START (由 npm run kg 自动生成，勿手改本标记区) -->

## 知识图谱与延伸阅读

> 本节由 `npm run kg` 自动生成（数据源 `knowledge-graph/data/graph.ts`）。要增删请改数据源后重跑。

### 本章概念图谱

> 节点：**橙框**=本章概念，蓝框=关联的其他章概念。连线按关系类型着色：前置(蓝) · 深化(紫) · 对比(玫红) · 应用(绿) · 组成(橙)。

```mermaid
graph LR
  classDef own fill:#fff7ed,stroke:#ea580c,stroke-width:3px,color:#7c2d12;
  classDef cross fill:#eef2ff,stroke:#6366f1,stroke-width:1.5px,color:#312e81;
  n_cp_compliance_policy_monitor_workflow["政策版本 diff"]
  n_cp_compliance_policy_monitor_quality["控制矩阵映射"]
  n_cp_compliance_policy_monitor_handoff["影响流程 owner"]
  n_cp_compliance_policy_monitor_workflow -->|前置| n_cp_compliance_policy_monitor_quality
  n_cp_compliance_policy_monitor_quality -->|前置| n_cp_compliance_policy_monitor_handoff
  class n_cp_compliance_policy_monitor_workflow,n_cp_compliance_policy_monitor_quality,n_cp_compliance_policy_monitor_handoff own;
  linkStyle 0 stroke:#2563eb,stroke-width:2px;
  linkStyle 1 stroke:#2563eb,stroke-width:2px;
```

### 延伸阅读

_暂无（可在 `graph.ts` 的 `ARTICLES` 中新增本章关联文章）。_

> 🗺️ 在[全局知识图谱](../../docs/knowledge-graph.md) / [交互式图谱](../../knowledge-graph/output/index.html) 中查看本章位置。

<!-- KG:END -->
